...
<div class="grid_15">
    <div class="box round first grid">
        ...
        <table class="data display datatable" id="example">
			...
			//rj
				$wp = strtoupper($rs_comments['wp_grade']);
                                if($rs_comments['mc'] > 0){
				$mc_client[$rs_comments['str_no']][$wp] += $rs_comments['mc'] ;
                                }
                                $dirt_client[$rs_comments['str_no']][$wp] += $rs_comments['dirt'] ;
                                $total_dirt_new += $rs_comments['dirt'];
                                $total_mc_new += $rs_comments['mc'];
			//end rj
                }
                ...
                if (trim($_SESSION['delivered_to']) != '') {
                    if ($netwt != '') {
                        ...
                        if ($row['str'] == '') {
                            ...
                            if ($row['is_marked'] == "yes") {
                                if ($row2['mc'] != '0') {
                                    echo "<td class='data' id='highlight'>" .$mc_client[$row['str']][$wpss] . "</td>";
                                } else {
                                    echo "<td class='data' id='highlight'></td>";
                                }
                            } else {
                                if ($row2['mc'] != '0') {
                                    echo "<td class='data' id='mc'>" . $mc_client[$row['str']][$wpss] . "</td>";
                                } else {
                                    echo "<td class='data' id='mc'></td>";
                                }
                            }
                        ...
                        if ($row['is_marked'] == 'yes') {
						   ...
						   ?><a title="Click to Erase Encoded Actual" onclick="return confirm('This Action was Disabled. Please contact RJ or Marlon.');"><img src='icon/bura.png' ></a><?php
                            echo "</td>";
                        } else {
                            echo "<td class='data'>";
...
//                    echo "<a id='" . $row['log_id'] . "' type='button' data-toggle='modal' data-target='#myModal2' onclick='typeNotation(this.id);'>" . $row['str'] . "</a>";
                    ...
                    echo "'";
                        echo "href='frmAddNetweight.php?str=" . $row['str'] . "&wp_grade=" . $row['wp_grade'] . "'>";} echo $row['str'] . "</a></u></td>";
                    echo "<td class='data'>" . $row['branch'] . "</td>";
                    echo "<td class='data'>" . $row2['delivered_to'] . "</td>";
                    $log_id = $row2['log_id'];
                    echo "<td class='data'>" . $row['trucking'] . "</td>";
                    ...
                    if ($row['str'] == '') {
                        ...
                        if ($row['is_marked'] == "yes") {
                            echo "<td class='data' id='highlight'>" . $dirt_client[$row['str']][$wpss] . "</td>";
                        } else {
                            echo "<td class='data' id='dirt'>" . $dirt_client[$row['str']][$wpss] . "</td>";
                        }
...
//                            echo "<button id='admin_" . $row['log_id'] . "' type='button' data-toggle='modal' data-target='#myModal' onclick='typeNotation(this.id);'>N</button>";
                           ...
                           // echo "<a href='delete_one_actual.php?log_id=" . $row2['log_id'] . "' title='Click to Erase Encoded Actual'><img src='icon/bura.png'></a>";
						   ?><a title="Click to Erase Encoded Actual" onclick="return confirm('This Action was Disabled. Please contact RJ or Marlon.');"><img src='icon/bura.png'></a><?php
							//echo "<img src='icon/bura.png'>";
//                            echo "<a rel='facebox' href='outgoing_frm_notation_branch.php?log_id=" . $row['log_id'] . "' title='Click to input notation.'>N</a>";
//                            echo "<button id='branch_" . $row['log_id'] . "' type='button' data-toggle='modal' data-target='#myModal' onclick='typeNotation(this.id);'>N</button>";
            ...
            echo "<td id = 'total'>" . number_format($total_dirt_new, 2) . "</td>";
