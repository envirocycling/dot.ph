<html>
<title>Update Supplier Pampanga</title>
	<body>
<?php
session_start();
include("config.php");
...
if(!empty($_GET['up_supp'])){
	...
	if(mysql_num_rows($sql_supp) > 0){
		...
		echo "<form action='http://192.168.1.149/ts/update_supp.php' method='POST' name='myForm'>";	
	...
	echo '<script>
			location.replace("http://192.168.1.149/ts/export_receiving_ims.php");
			</script>';
}else{
