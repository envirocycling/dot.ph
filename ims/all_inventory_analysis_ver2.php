...
//                echo "last month: ".$last_month." last last day: ".$last_last_day." last first day: ".$last_1st_day;
                ...
                foreach($wp_array as $wp_grade) {
                    ...
                    if($wp_grade!='LCWL' && $wp_grade!='CHIPBOARD'  && $wp_grade!='LCCB') {
                        $query21="SELECT sum(weight) from month_end_loose where wp_grade='$wp_grade' and branch like '%$branch%' and month_end_date='$last_month'";
                    } else {
                        if($wp_grade=='LCWL') {
                            $query21="SELECT sum(weight) from month_end_loose where wp_grade='LCWL' and branch like '%$branch%' and month_end_date='$last_month'";
                        } else if($wp_grade=='CHIPBOARD' || $wp_grade=='LCCB') {
                            $query21="SELECT sum(weight) from month_end_loose where  (wp_grade='CHIPBOARD' or wp_grade='cb') and branch like '%$branch%' and month_end_date='$last_month'";
                        }
                    }
                    ...
                    $query10="SELECT sum(weight) FROM loose_papers where date='$to' and wp_grade='$wp_grade' and branch like '%$branch%'" ;

                    $result10=mysql_query($query10);
                    if($row10= mysql_fetch_array($result10)) {
                        $loose=$row10['sum(weight)'];
                    }
                    if($wp_grade!='LCWL' && $wp_grade!='CHIPBOARD'  && $wp_grade!='LCCB') {
                        $query22="SELECT sum(weight) from month_end_loose where wp_grade='$wp_grade' and branch like '%$branch%' and month_end_date='$month'";
                    }else {
                        if($wp_grade=='LCWL') {
                            $query22="SELECT sum(weight) from month_end_loose where wp_grade='LCWL' and branch like '%$branch%' and month_end_date='$month'";
                        } else if($wp_grade=='CHIPBOARD' || $wp_grade=='LCCB') {
                            $query22="SELECT sum(weight) from month_end_loose where  (wp_grade='CHIPBOARD' or wp_grade='cb') and branch like '%$branch%' and month_end_date='$month'";
                        }
                    }
