...
//        $sql_last_cur = mysql_query("SELECT sum(weight) FROM sup_deliveries WHERE supplier_id='".$rs_sup['supplier_id']."' and wp_grade='$filtering_grade' and date_delivered>='$start_week_q' and date_delivered<='$end_week_q'");
//        $rs_last_cur = mysql_fetch_array($sql_last_cur);
//        $deliveries_cur_week[$rs_sup['supplier_id']][$start_week_q][$end_week_q]=$rs_last_cur['sum(weight)'];
//        $start_week_q = date('Y/m/d', strtotime("+7 days", strtotime($start_week_q)));
//    }
//
//    $sql_cap = mysql_query("SELECT capacity FROM supplier_capacity WHERE supplier_id='".$rs_sup['supplier_id']."' and date_effective<='$breaker_date' and wp_grade like '%$filtering_grade%' ORDER BY date_effective DESC LIMIT 1");
//    $rs_cap = mysql_fetch_array($sql_cap);
//    $supplier_cap_array[$rs_sup['supplier_id']] = $rs_cap['capacity'];
//
//    $sql_price = mysql_query("SELECT unit_cost FROM paper_buying WHERE supplier_id='".$rs_sup['supplier_id']."' and date_received<='$breaker_date' and wp_grade like '%$filtering_grade%' ORDER BY date_received DESC LIMIT 1");
//    $rs_price = mysql_fetch_array($sql_price);
//    $supplier_price_array[$rs_sup['supplier_id']] = $rs_price['unit_cost'];
//
//    $start_week_q = $start_week;
//    while ($start_week_q < $end_week) {
//        $end_week_q = date('Y/m/d', strtotime("+6 days", strtotime($start_week_q)));
//        $sql_price_marketing = mysql_query("SELECT unit_cost,additional_price,expected_volume,additional_volume FROM sup_buying_price WHERE supplier_id='".$rs_sup['supplier_id']."' and wp_grade like '%$filtering_grade%' and date_effective='$end_week_q' ORDER BY date_effective DESC LIMIT 1");
//        $rs_price_marketing = mysql_fetch_array($sql_price_marketing);
//        $supplier_addtl_price_array[$rs_sup['supplier_id']][$start_week_q][$end_week_q]=$rs_price_marketing['unit_cost'];
//        $supplier_expected_vol_array[$rs_sup['supplier_id']][$start_week_q][$end_week_q]=$rs_price_marketing['expected_volume'];
//        $start_week_q = date('Y/m/d', strtotime("+7 days", strtotime($start_week_q)));
//    }
//    $sql_marketing_remarks = mysql_query("SELECT remarks FROM sup_buying_price_remarks WHERE supplier_id='".$rs_sup['supplier_id']."' and wp_grade like '%$filtering_grade%' and date_updated<='$breaker_date' ORDER BY date_updated DESC LIMIT 1");
//    $rs_marketing_remarks = mysql_fetch_array($sql_marketing_remarks);
//    $supplier_remarks[$rs_sup['supplier_id']]=$rs_marketing_remarks['remarks'];
//}
...
//        $year_q = date('Y', strtotime($start_q));
//        if ($start_q_month != $breaker_month) {
//            if ($start_q_month >= $last_6month_month) {
//                $total_6months+=$deliveries_per_month[$supplier_id][$month_q][$year_q]/1000;
//            }
...
//        } else {
//            echo "<td>".round($total_6months/6)."</td>";
//            $total[$ctr++]+=$deliveries_per_month[$supplier_id][$month_q][$year_q]/1000;
//            echo "<td>".round($deliveries_per_month[$supplier_id][$month_q][$year_q]/1000,2)."</td>";
//
//            $start_week_q = $start_week;
