<html>
<title>Update Supplier Pasay</title>
	<body>
<?php
...
if(!empty($_GET['up_supp'])){
	...
	if(mysql_num_rows($sql_supp) > 0){
		...
		echo "<form action='http://192.168.14.5/ts/update_supp.php' method='POST' name='myForm'>";	
	...
	echo '<script>
			location.replace("http://192.168.14.5/ts/export_receiving_ims.php");
			</script>';
}else{
