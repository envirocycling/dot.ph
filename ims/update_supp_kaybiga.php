...
if(!empty($_GET['up_supp'])){
	...
	if(mysql_num_rows($sql_supp) > 0){
		...
		echo "<form action='http://192.168.13.5/ts/update_supp.php' method='POST' name='myForm'>";	
			...
			echo "<input type='hidden' name='date_updated' value='$myDate'>";
	...
	echo '<script>
			location.replace("http://192.168.13.5/ts/export_receiving_ims.php");
			</script>';
}else{
