        ...
        <h2><?php 
            ...
            <table class="data display datatable" id="example">
                ...
                while($row40 = mysql_fetch_array($result40)) {
                    $supplier_details_array[$row40['supplier_id']]=($row40['supplier_name']."+".$row40['branch']."+".$row40['style']."+".$row['description']);
                }
                ...
                if(trim($filtering_grade)!='') {
                    if($filtering_branch=='Sauyo/Kaybiga') {
                        $query3="SELECT supplier_details.supplier_id as supplier_id,sum(weight),date_delivered,month_delivered FROM sup_deliveries  join supplier_details on sup_deliveries.supplier_id = supplier_details.supplier_id where supplier_details.status!='inactive' and month_delivered='$weekly_month' and year_delivered='$weekly_year' and wp_grade = '$filtering_grade' and   (branch_delivered ='Kaybiga' or branch_delivered ='Sauyo')  and date_delivered >= '$start_date' and date_delivered <='$breaker_date' group by supplier_id,date_delivered";
                    }else {
                        $query3="SELECT supplier_details.supplier_id as supplier_id,sum(weight),date_delivered,month_delivered FROM sup_deliveries  join supplier_details on sup_deliveries.supplier_id = supplier_details.supplier_id where supplier_details.status!='inactive' and  month_delivered='$weekly_month' and year_delivered='$weekly_year' and wp_grade = '$filtering_grade' and branch_delivered like '%".$filtering_branch."%' and date_delivered >= '$start_date' and date_delivered <='$breaker_date' group by supplier_id,date_delivered";
                    }
                }else {
                    if($filtering_branch=='Sauyo/Kaybiga') {
                        $query3="SELECT supplier_details.supplier_id as supplier_id,sum(weight),date_delivered,month_delivered FROM sup_deliveries  join supplier_details on sup_deliveries.supplier_id = supplier_details.supplier_id where supplier_details.status!='inactive' and  month_delivered='$weekly_month' and year_delivered='$weekly_year'  and   (branch_delivered ='Kaybiga' or branch_delivered ='Sauyo') and date_delivered >= '$start_date' and date_delivered <='$breaker_date' group by supplier_id,date_delivered";
                    }else {
                        $query3="SELECT supplier_details.supplier_id as supplier_id,sum(weight),date_delivered,month_delivered FROM sup_deliveries  join supplier_details on sup_deliveries.supplier_id = supplier_details.supplier_id where supplier_details.status!='inactive' and  month_delivered='$weekly_month' and year_delivered='$weekly_year'  and branch_delivered like '%".$filtering_branch."%' and date_delivered >= '$start_date' and date_delivered <='$breaker_date' group by supplier_id,date_delivered";
                    }
                }
