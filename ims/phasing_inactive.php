...
$deliveries_per_month_phas1=array();
$deliveries_per_month_phas2=array();
$total_quota=0;
$total_perf=0;
?>
<script>
    ...
    <div class="box round first grid">
        <h2><?php
            ...
            if(strtoupper($filtering_branch)!='') {
                if($filtering_grade!='') {
                    echo "<h2>".strtoupper($filtering_branch)." Suppliers Quota Phase Monitoring for the period <u>$start_date to $breaker_date</u> in MT on ".$filtering_grade."</h2>";
                }else {
                    echo "<h2>".strtoupper($filtering_branch)." Suppliers Quota Phase Monitoring for the period <u>$start_date to $breaker_date</u>  in MT on all grades</h2>";
                }
            }else {
                if($filtering_grade!='') {
                    echo "<h2>CONSOLIDATED Suppliers Quota Phase Monitoring for the period <u>$start_date to $breaker_date</u> in MT on wp grade: ".$filtering_grade."</h2>";
                }else {
                    echo "<h2>CONSOLIDATED Suppliers Quota Phase Monitoring for the period <u>$start_date to $breaker_date</u> in MT on all grades</h2>";
                }
            }
            ?>
            ...
            <table class="data display datatable" id="example">
                ...
                $result=mysql_query($query);
                while($row = mysql_fetch_array($result)) {
                    array_push($weeks_array,$row['month_delivered']."-".$row['year_delivered']."_"."Phase 1");
                    array_push($weeks_array,$row['month_delivered']."-".$row['year_delivered']."_"."Phase 2");
                    array_push($months_array,$row['month_delivered']."-".$row['year_delivered']."_"."Phase 1");
                    array_push($months_array,$row['month_delivered']."-".$row['year_delivered']."_"."Phase 2");
                }

                if($filtering_branch=='Sauyo/Kaybiga') {
                    $query5="SELECT sum(weight),month_delivered,supplier_id,date_delivered,year_delivered from sup_deliveries where  day_delivered<=15 and date_delivered >='$start_date' and  date_delivered <='$breaker_date' and wp_grade like '%".$filtering_grade."%' and (branch_delivered ='Kaybiga' or branch_delivered ='Sauyo')  group by  supplier_id,month_delivered,year_delivered order by date_delivered";
                }else {
                    $query5="SELECT sum(weight),month_delivered,supplier_id,date_delivered,year_delivered from sup_deliveries where   day_delivered<=15 and  date_delivered >='$start_date' and date_delivered <='$breaker_date' and wp_grade like '%".$filtering_grade."%' and branch_delivered like '%".$filtering_branch."%' group by  supplier_id,month_delivered,year_delivered order by date_delivered";
                }
                $result5 =mysql_query($query5);
                while($row5 = mysql_fetch_array($result5)) {
                    $deliveries_per_month_phase1[$row5['supplier_id']."+".ucfirst(strtolower($row5['month_delivered']))."-".$row5['year_delivered']]=(($row5['sum(weight)'])/1000);
                }


                if($filtering_branch=='Sauyo/Kaybiga') {
                    $query52="SELECT sum(weight),month_delivered,supplier_id,date_delivered,year_delivered from sup_deliveries where   day_delivered > 15 and date_delivered >='$start_date' and  date_delivered <='$breaker_date' and wp_grade like '%".$filtering_grade."%' and (branch_delivered ='Kaybiga' or branch_delivered ='Sauyo')  group by  supplier_id,month_delivered,year_delivered order by date_delivered";
                }else {
                    $query52="SELECT sum(weight),month_delivered,supplier_id,date_delivered,year_delivered from sup_deliveries where  day_delivered >15 and  date_delivered >='$start_date' and date_delivered <='$breaker_date' and wp_grade like '%".$filtering_grade."%' and branch_delivered like '%".$filtering_branch."%' group by  supplier_id,month_delivered,year_delivered order by date_delivered";
                }
                $result52 =mysql_query($query52);
                while($row52 = mysql_fetch_array($result52)) {
                    $deliveries_per_month_phase2[$row52['supplier_id']."+".ucfirst(strtolower($row52['month_delivered']))."-".$row52['year_delivered']]=(($row52['sum(weight)'])/1000);
                }
                ...
                $quota_array=array();
                $total_quota=0;
                $query40="SELECT incentive_scheme.sup_id as sup_id,incentive_scheme.quota as quota,start_date as start_date,end_date ,wp_grade,sup_id,incentive_scheme.scheme as scheme from incentive_scheme where  incentive_scheme.end_date>='$breaker_date' and incentive_scheme.remarks ='' " ;
                $result40=mysql_query($query40);
                while($row40 = mysql_fetch_array($result40)) {
                ...
                if(trim($filtering_grade)!='') {
                    if($filtering_branch=='Sauyo/Kaybiga') {
                        $query3="SELECT supplier_id,sum(weight),date_delivered,month_delivered FROM sup_deliveries  where month_delivered='$weekly_month' and year_delivered='2014' and wp_grade = '$filtering_grade' and   (branch_delivered ='Kaybiga' or branch_delivered ='Sauyo')  and date_delivered >= '$start_date' and date_delivered <='$breaker_date' group by supplier_id,date_delivered";
                    }else {
                        $query3="SELECT supplier_id,sum(weight),date_delivered,month_delivered FROM sup_deliveries  where month_delivered='$weekly_month' and year_delivered='2014' and wp_grade = '$filtering_grade' and branch_delivered like '%".$filtering_branch."%' and date_delivered >= '$start_date' and date_delivered <='$breaker_date' group by supplier_id,date_delivered";
                    }
                }else {
                    if($filtering_branch=='Sauyo/Kaybiga') {
                        $query3="SELECT supplier_id,sum(weight),date_delivered,month_delivered FROM sup_deliveries  where month_delivered='$weekly_month' and year_delivered='2014'  and   (branch_delivered ='Kaybiga' or branch_delivered ='Sauyo') and date_delivered >= '$start_date' and date_delivered <='$breaker_date' group by supplier_id,date_delivered";
                    }else {
                        $query3="SELECT supplier_id,sum(weight),date_delivered,month_delivered FROM sup_deliveries  where month_delivered='$weekly_month' and year_delivered='2014'  and branch_delivered like '%".$filtering_branch."%' and date_delivered >= '$start_date' and date_delivered <='$breaker_date' group by supplier_id,date_delivered";
                    }
                }
...
//////////////////////////////////
                    ...
                    foreach($months_array as $months) {
                        ...
                        $key= preg_split("[_]",$months);
                        $key_month=$key[0];
                        $key_phase=$key[1];

                        $key_finder2=$value2."+".$key_month;
                        $filtering_tool=$value2."_".$key_month;

                        if($key_phase=='Phase 1') {
                            if(!empty($deliveries_per_month_phase1[$key_finder2])) {
                                echo "<td >".round($deliveries_per_month_phase1[$key_finder2],2)."</td>";
                            }
                            else {
                                echo "<td >-</td>";
                            }
                        }else {
                            if(!empty($deliveries_per_month_phase2[$key_finder2])) {
                                echo "<td >".round($deliveries_per_month_phase2[$key_finder2],2)."</td>";
                            }
                            else {
                                echo "<td >-</td>";
                            }
                ...
                $total_deliveries_phase1=array();
                $total_deliveries_phase2=array();

                if($filtering_branch=='Sauyo/Kaybiga') {
                    $query5="SELECT sum(weight),month_delivered,year_delivered from sup_deliveries where day_delivered<=15 and date_delivered >='$start_date' and  date_delivered <='$breaker_date' and wp_grade like '%".$filtering_grade."%' and (branch_delivered ='Kaybiga' or branch_delivered ='Sauyo')  group by  month_delivered,year_delivered order by date_delivered";
                }else {
                    $query5="SELECT sum(weight),month_delivered,year_delivered from sup_deliveries where day_delivered<=15 and  date_delivered >='$start_date' and date_delivered <='$breaker_date' and wp_grade like '%".$filtering_grade."%' and branch_delivered like '%".$filtering_branch."%' group by  month_delivered,year_delivered order by date_delivered";
                }
                $result5 =mysql_query($query5);
                while($row5 = mysql_fetch_array($result5)) {
                    $total_deliveries_phase1[$row5['month_delivered']."-".$row5['year_delivered']]=$row5['sum(weight)']/1000;
                }



                if($filtering_branch=='Sauyo/Kaybiga') {
                    $query53="SELECT sum(weight),month_delivered,year_delivered from sup_deliveries where  day_delivered > 15 and date_delivered >='$start_date' and  date_delivered <='$breaker_date' and wp_grade like '%".$filtering_grade."%' and (branch_delivered ='Kaybiga' or branch_delivered ='Sauyo')  group by  month_delivered,year_delivered order by date_delivered";
                }else {
                    $query53="SELECT sum(weight),month_delivered,year_delivered from sup_deliveries where   day_delivered > 15 and  date_delivered >='$start_date' and date_delivered <='$breaker_date' and wp_grade like '%".$filtering_grade."%' and branch_delivered like '%".$filtering_branch."%' group by  month_delivered,year_delivered order by date_delivered";
                }
                $result53 =mysql_query($query53);
                while($row53 = mysql_fetch_array($result53)) {
                    $total_deliveries_phase2[$row53['month_delivered']."-".$row53['year_delivered']]=$row53['sum(weight)']/1000;
                }
                ...
                foreach($months_array as $var) {
                    $var_details=preg_split("[_]",$var);

                    if ($var_details[1]=='Phase 1') {
                        if(!empty($total_deliveries_phase1[$var_details[0]])) {
                            echo "<td class='TOTAL' >".round($total_deliveries_phase1[$var_details[0]],2)."</td>";
                        }else {
                            echo "<td class='TOTAL'></td>";
                        }

                    }else {
                        if(!empty($total_deliveries_phase2[$var_details[0]])) {
                            echo "<td class='TOTAL' >".round($total_deliveries_phase2[$var_details[0]],2)."</td>";
                        }else {
                            echo "<td class='TOTAL'></td>";
                        }
                    }
