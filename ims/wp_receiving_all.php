          ...
          echo "<th id='total'>TOTAL</th>";
            ...
            if($_SESSION['usertype']=='Super User') {
                if($_SESSION['selected_grade']=='all') {
                    $query="SELECT supplier_name,count(supplier_name) FROM sup_deliveries group by supplier_name order by supplier_name asc,del_id asc;";
                }else {
                    $query="SELECT supplier_name,count(supplier_name) FROM sup_deliveries group by supplier_name order by supplier_name asc,del_id asc;";
                }
            }
            ...
            if($_SESSION['usertype']=='Super User') {
                if($_SESSION['selected_grade']=='all') {
                    $query3="SELECT supplier_id,del_id,supplier_name,sum(weight),month_delivered FROM sup_deliveries   group by supplier_name,month_delivered order by supplier_name asc,del_id asc;";
                }else {
                    $query3="SELECT supplier_id,del_id,supplier_name,sum(weight),month_delivered FROM sup_deliveries   group by supplier_name,month_delivered order by supplier_name asc,del_id asc;";
                }
            }
