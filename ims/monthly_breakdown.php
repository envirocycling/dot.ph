...
<script>
    ...
    function showCapacity(str) {
        ...
        //        window.open("not_available.php?parameter=" + str, 'mywindow', 'width=600,height=500');]
        //        window.open("view_assessment_history.php?sup_id=" + str, 'mywindow', 'width=1000,height=400');
        var splits = str.split("_");
        window.open("editSupplier.php?sup_id="+splits[0]+'&wp_grade='+splits[1],'mywindow','width=1000,height=650');
    }

    function insertvolume(str) {
        window.open("frm_insert_volume.php?id=" + str, 'mywindow', 'width=400,height=300');
    }

    function editvolume(str) {
        window.open("frm_edit_volume.php?id=" + str, 'mywindow', 'width=500,height=400');
    }
    function openWindow(str) {
        var x = screen.width / 2 - 700 / 2;
        var y = screen.height / 2 - 450 / 2;
        window.open("view_supplier_to_delete.php?sup_id=" + str, 'mywindow', 'width=1000,height=650');
    }
    function showAssessment(str) {
        var x = screen.width / 2 - 700 / 2;
        var y = screen.height / 2 - 450 / 2;
        window.open("view_assessment_history.php?sup_id=" + str, 'mywindow', 'width=1000,height=400');
    }
...
$current_yday = date("d", strtotime($breaker_date));
$day_percentage = $current_yday / $last_day_of_month;
...
$total_current_month_vs_prev_month = 0;
?>
<div class="grid_16" >
            ...
            <table class="data display datatable" id="example">
                ...
                if ($filtering_branch == 'Sauyo/Kaybiga') {
                    ...
                    while ($row40 = mysql_fetch_array($result40)) {
                        $supplier_details_array[$row40['supplier_id']] = ($row40['supplier_name'] . "+" . $row40['branch'] . "+" . $row40['style'] . "+" . $row40['description'] . "+" . $row40['street'] . "+" . $row40['municipality'] . "+" . $row40['province'] . "+" . $row40['supplier_id']);
                    }
                    $result = mysql_query($query);
                    $month_counter = 0;
                    $last_month = 0;
...
//                    echo "<th>Desc</th>";
                    ...
                    echo "<th>Current Month VS Prev. Month (Based on MTD)</th>";
                    echo "<th>EFI Buying Price</th>";
//                    echo "<th>Assessment</th>";
                    echo "<th>Capacity</th>";
                    echo "<th>Delivers To</th>";
////                    echo "<th>Competitor's Price</th>";
                    echo "<th>Calculated: Tons We Are Not Getting</th>";
//                    echo "<th>Defined: Tons We Are Not Getting</th>";
                    ...
                    $delivers_to_array = array();
                    $competitors_price_array = array();

                    $capacity_j1 = array ();
                    $capacity_check_j1 = array ();
                    $type_j1 = array ();
                    $del_to_j1 = array ();
                    $del_to_price_j1 = array ();
                    $del_to_vol_j1 = array ();
                    ...
                    foreach ($supplier_array as $sup_id) {
                        if ($filtering_grade != '') {
                            $capacity_check_j1[$sup_id]='0';
                            $sql_cap = mysql_query("SELECT * FROM supplier_capacity WHERE wp_grade='$filtering_grade' and supplier_id='$sup_id' and date_effective<='$breaker_date' ORDER BY date_effective DESC");
                            $rs_cap = mysql_fetch_array($sql_cap);
                            $capacity_j1[$sup_id] = $rs_cap['capacity'];
                            $grades_array = array($filtering_grade);
                            $capa = '';
                            foreach ($grades_array as $grade) {
                                $sql_cap = mysql_query("SELECT * FROM supplier_capacity WHERE wp_grade='$grade' and supplier_id='$sup_id' and date_effective<='$breaker_date' order by date_effective desc LIMIT 1");
                                $rs_cap = mysql_fetch_array($sql_cap);
                                if ($rs_cap['capacity']=='') {
                                    $capacity_check_j1[$sup_id]='1';
                                }
                            ...
                            foreach ($grades_array as $grade) {
                                $ctr = 0;
                                $sql_del_to = mysql_query("SELECT * FROM supplier_assessment WHERE wp_grade='$grade' and supplier_id='$sup_id' and date<='$breaker_date' and status!='deleted'");
                                while ($rs_del_to = mysql_fetch_array($sql_del_to)) {
                                    $del_to_j1[$sup_id][$grade][$ctr]=$rs_del_to['deliver_to'];
                                    $del_to_price_j1[$sup_id][$grade][$ctr]=$rs_del_to['price'];
                    ...
                    if ($filtering_grade == '') {
                        $query21 = "SELECT * FROM monthly_remarks where date_inputed>='$start_date' and date_inputed<='$breaker_date' and wp_grade = '$filtering_grade' order by log_id asc ";
                        $result21 = mysql_query($query21);
                        while ($row21 = mysql_fetch_array($result21)) {
                    ...
                    if (trim($filtering_grade) != '') {
                        if ($filtering_branch == 'Sauyo/Kaybiga') {
                            $query3 = "SELECT sup_deliveries.supplier_id as supplier_id,sum(weight),date_delivered,month_delivered FROM sup_deliveries  join supplier_details on sup_deliveries.supplier_id=supplier_details.supplier_id where supplier_details.inter_branch='0' and supplier_details.status!='inactive' and month_delivered='$weekly_month' and year_delivered='$weekly_year' and wp_grade = '$filtering_grade' and   (branch ='Kaybiga' or branch ='Sauyo')  group by supplier_details.supplier_id,date_delivered";
                        } else {
                            $query3 = "SELECT sup_deliveries.supplier_id as supplier_id,sum(weight),date_delivered,month_delivered FROM sup_deliveries  join supplier_details on sup_deliveries.supplier_id=supplier_details.supplier_id where supplier_details.inter_branch='0' and supplier_details.status!='inactive' and month_delivered='$weekly_month' and year_delivered='$weekly_year' and wp_grade = '$filtering_grade' and branch like '%" . $filtering_branch . "%' group by supplier_details.supplier_id,date_delivered";
                        }
                    } else {
                        if ($filtering_branch == 'Sauyo/Kaybiga') {
                            $query3 = "SELECT sup_deliveries.supplier_id as supplier_id,sum(weight),date_delivered,month_delivered FROM sup_deliveries  join supplier_details on sup_deliveries.supplier_id=supplier_details.supplier_id where supplier_details.inter_branch='0' and supplier_details.status!='inactive' and month_delivered='$weekly_month' and year_delivered='$weekly_year'  and   (branch ='Kaybiga' or branch ='Sauyo') group by supplier_details.supplier_id,date_delivered";
                        } else {
                            $query3 = "SELECT sup_deliveries.supplier_id as supplier_id,sum(weight),date_delivered,month_delivered FROM sup_deliveries  join supplier_details on sup_deliveries.supplier_id=supplier_details.supplier_id where supplier_details.inter_branch='0' and supplier_details.status!='inactive' and month_delivered='$weekly_month' and year_delivered='$weekly_year'  and branch like '%" . $filtering_branch . "%' group by supplier_details.supplier_id,date_delivered";
                        }
                    }
...
//                            echo "<td id='".$value2."_".$filtering_grade."' onclick='showAssessment(this.id);' class='data' >" . $value2 . "</td>";
...
//                            echo "<td id='" . $value2 . "' onclick='openWindow(this.id);' title='delete'>Delete</td>";
                            if ($filtering_branch != '') {
//                                echo "<td id='" . $value2 . "' onclick='openWindow(this.id);' title='delete'>Delete</td>";

                                $sql = mysql_query("SELECT * FROM sup_starting_volume WHERE supplier_id='" . $supplier_details[7] . "' and branch='$filtering_branch'");
                                $rs = mysql_fetch_array($sql);
                                $starting_volume = $rs['starting_volume'];
                                $starting_volume_date = $rs['starting_volume_date'];
                                if (!empty($starting_volume) || $starting_volume == '0') {
                                    echo "<td id='" . $supplier_details[7] . "_" . $starting_volume_date . "_" . $starting_volume . "_" . $filtering_branch . "_" . $breaker_date . "_" . $supplier_details[0] . "' onclick='editvolume(this.id);'>" . $starting_volume . "</td>";
                                } else {
                                    echo "<td id='" . $supplier_details[7] . "_" . $filtering_branch . "_" . $breaker_date . "_" . $supplier_details[0] . "' onclick='insertvolume(this.id);'>Insert Volume</td>";
                                }
                            }
                            echo "<td id='left_header' >" . $supplier_details[1] . "</td>";
                            if ($supplier_details[4] != '') {
                                echo "<td id='left_header' >" . $supplier_details[4] . "</td>";
                            } else {
                                echo "<td id='left_header' ></td>";
                            }
                            if ($supplier_details[5] != '') {
                                echo "<td id='left_header' >" . $supplier_details[5] . "</td>";
                            } else {
                                echo "<td id='left_header' ></td>";
                            }
                            if ($supplier_details[6] != '') {
                                echo "<td id='left_header' >" . $supplier_details[6] . "</td>";
                            } else {
                                echo "<td id='left_header' ></td>";
                            }
...
////////////////////////////////////////////////////////////////////////////////////////
                        ...
                        foreach ($months_array as $months) {
                            ...
                            if (!empty($deliveries_per_month[$key_finder2])) {
                                ...
                                if ($months == $last_month) {
                                    ...
                                    $last_month_delivery = $week_for_comparision;
                                }
                        ...
                        echo "<td  id='variance'>" . round(($five_week_total) - ($last_month_delivery * $day_percentage), 2) . "</td>";
                        $total_current_month_vs_prev_month+=round(($five_week_total) - ($last_month_delivery * $day_percentage), 2);
...
//                        echo "<td id='".$value2."' onclick='showAssessment(this.id);'><i style='color:blue; font-size:11'><u>View</u></i></td>";
                        $calculated_tons_we_are_not_getting=0;
                        $based_on_date_capacity = round($capacity_j1[$value2] * $day_percentage, 0);
                        $calculated_tons_we_are_not_getting = round($based_on_date_capacity - $five_week_total, 0);
                        if ($capacity_check_j1[$value2]!=1) {
                            echo " <td class='capacity' id='" . $value2 . "_" . $filtering_grade . "_" . $capacity_j1[$value2] . "' onclick='showCapacity(this.id);'>
                              ".$capacity_j1[$value2]."</td>";
                        } else {
                            echo "<td class='capacity' id='" . $value2 . "_" . $filtering_grade . "_" . $capacity_j1[$value2] . "' onclick='showCapacity(this.id);'>
                              <i>Unknown Capacity</i></td>";
                        }
                        ...
                        foreach ($grades_array as $grade) {
                            $ctr = 0;
                            if ($sup_del_count_j1[$value2][$grade] != 0) {
                                echo "<table class='assess'>";
                                echo "<td colspan='4' class='assess' align='center'>".strtoupper($grade)."</td>";
                                echo "<tr>";
                                echo "<td class='assess'>Delivers To</td>";
                                echo "<td class='assess'>Price</td>";
                                echo "<td class='assess'>Volume</td>";
                                echo "<td class='assess'>Type</td>";
                                echo "</tr>";
                                while ($ctr < $sup_del_count_j1[$value2][$grade]) {
                                    if (!empty ($del_to_j1[$value2][$grade][$ctr])) {
                                        $sql_del_to = mysql_query("SELECT * FROM supplier_details WHERE supplier_id='".$del_to_j1[$value2][$grade][$ctr]."'");
                                        $rs_del_to = mysql_fetch_array($sql_del_to);
                                        echo "<tr>";
                                        echo "<td class='assess'>".$rs_del_to['supplier_name']."&nbsp;&nbsp;&nbsp;</td>";
                                        echo "<td class='assess'>".$del_to_price_j1[$value2][$grade][$ctr]."&nbsp;&nbsp;&nbsp;</td>";
                                        echo "<td class='assess'>".$del_to_vol_j1[$value2][$grade][$ctr]."&nbsp;&nbsp;&nbsp;</td>";
                                        echo "<td class='assess'>".strtoupper($del_to_type_j1[$value2][$grade][$ctr])."&nbsp;&nbsp;&nbsp;</td>";
                                        echo "</tr>";
                                    }
                                    $ctr++;
                                }
                        ...
                        echo "<br>";
                        echo "<a href='editSupplier.php?sup_id=" . $value2 . "&wp_grade=" . $filtering_grade . "'><u style='color:blue; font-size:10'><i>input</i></u></a>";
                        echo "</td>";
//                            echo " <td class='capacity' id='" . $value2 . "_" . $filtering_grade . "_" . $capacity_j1[$value2] . "' onclick='showCapacity(this.id);'>
//                              </td>";
                        echo " <td class='capacity' id='" . $value2 . "_" . $filtering_grade . "_" . $capacity_j1[$value2] . "' onclick='showCapacity(this.id);'>
                                $calculated_tons_we_are_not_getting</td>";
...
//                    echo "<td class='TOTAL' ></td>";
                    ...
                    foreach ($months_array as $var) {
                        ...
                        if (trim($filtering_grade) != '') {
                            if ($filtering_branch == 'Sauyo/Kaybiga') {
                                $query31 = "SELECT sum(weight)/1000 as weight, month_delivered,year_delivered,date_delivered FROM sup_deliveries join supplier_details on sup_deliveries.supplier_id=supplier_details.supplier_id where supplier_details.inter_branch='0' and supplier_details.status!='inactive' and wp_grade='" . $filtering_grade . "' and year_delivered='$year' and month_delivered ='$month'  and   (supplier_details.branch ='Kaybiga' or supplier_details.branch ='Sauyo')  ";
                            } else {
                                $query31 = "SELECT sum(weight)/1000 as weight, month_delivered,year_delivered,date_delivered FROM sup_deliveries join supplier_details on sup_deliveries.supplier_id=supplier_details.supplier_id where supplier_details.inter_branch='0' and supplier_details.status!='inactive' and   wp_grade='" . $filtering_grade . "' and year_delivered='$year' and month_delivered ='$month' and supplier_details.branch like '%" . $filtering_branch . "%'  ";
                            }
                        } else {
                            if ($filtering_branch == 'Sauyo/Kaybiga') {
                                $query31 = "SELECT sum(weight)/1000 as weight, month_delivered,year_delivered,date_delivered FROM sup_deliveries join supplier_details on sup_deliveries.supplier_id=supplier_details.supplier_id where supplier_details.inter_branch='0' and supplier_details.status!='inactive' and  year_delivered='" . $year . "' and month_delivered ='$month' and (supplier_details.branch ='Kaybiga' or supplier_details.branch ='Sauyo') ";
                            } else {
                                $query31 = "SELECT sum(weight)/1000 as weight, month_delivered,year_delivered,date_delivered FROM sup_deliveries join supplier_details on sup_deliveries.supplier_id=supplier_details.supplier_id where supplier_details.inter_branch='0' and supplier_details.status!='inactive' and  year_delivered='" . $year . "' and month_delivered ='$month'   and supplier_details.branch like '%" . $filtering_branch . "%'  ";
                            }
                        }
                        ...
                        if ($row31 = mysql_fetch_array($result31)) {
                            ...
                            if ($l6m_month == $month && $l6m_year == $year) {
                                $l6m_total+=$row31['weight'];
...
//                    $l6m_total=0;
//                    $l6m = date('Y/m', strtotime("-6 monthS", strtotime($breaker_date)));
//                    $l6m = $l6m . "/01";
//                    $l6m_end = date('Y/m/t', strtotime("-1 month", strtotime($breaker_date)));
//                    while ($l6m <= $l6m_end){
//                        $month_l6m = date("F", strtotime($l6m));
//                        $year_l6m = date("Y", strtotime($l6m));
//                        $sql_query = mysql_query("SELECT sum(weight) FROM sup_deliveries WHERE month_delivered='$month_l6m' and year_delivered='$year_l6m' and wp_grade like '%$filtering_grade%' and branch_delivered like '%$filtering_branch%'");
//                        $rs_query = mysql_fetch_array($sql_query);
//                        echo $rs_query['sum(weight)']/1000;
//                        echo "<br>";
//                        $l6m_total += $rs_query['sum(weight)']/1000;
//                        $l6m = date('Y/m/d', strtotime("+1 month", strtotime($l6m)));
//                    }
                    $l6m_total = $l6m_total / 6;
                    echo "<td id='last_months_avg' >" . round($l6m_total, 2) . "</td>";
//                      echo "<td id='last_months_avg' >".round($last_6_months_total,2)."</td>";
                    echo "<td id='current_month' >" . round($current_month_total, 2) . "</td>";
                    echo "<td id='variance' >" . round(($current_month_total) - ($l6m_total * $day_percentage), 2) . "</td>";
//                    echo "<td id='variance' >".round(($current_month_total)-($last_6_months_total*$day_percentage),2)."</td>";
                    ...
                    echo "<td id='variance' >" . round($total_current_month_vs_prev_month, 2) . "</td>";
