...
$query="SELECT branch from outgoing where branch !='' and date like '%$month%' group by branch ";
...
foreach($branches_array as $branch_name) {
    echo fromLocWeightGiver($branch_name,$month,$as_of).",";
}
...
<?php
foreach($branches_array as $branch_name) {
    echo ($actual_all_branch[$branch_name]- fromLocWeightGiver($branch_name,$month,$as_of)).",";
}
