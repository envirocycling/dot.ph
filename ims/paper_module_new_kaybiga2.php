        ...
        $plate_number = $_POST['plate_number' . $ctr];
  		...
  		if ($weight >= 0 && $new_price > 0) {
			...
			if(mysql_num_rows($chk) == 0) {	
				...
				if (mysql_query("INSERT INTO paper_buying (trans_id,detail_id,date_received,priority_number,supplier_id,supplier_name,plate_number,wp_grade,corrected_weight,unit_cost,paper_buying,branch) VALUES ('$tid','$did','$date','$priority_number','$supplier_id','$supplier_name','$plate_number','$wp_grade','$weight','$new_price','$paper_buying','$branch')") or die(mysql_error())) {

                	$num = 1;
				}             
			}else if(mysql_num_rows($chk) > 0) {
					
				if(mysql_query("UPDATE paper_buying SET trans_id='$tid',detail_id='$did',date_received='$date',priority_number='$priority_number',supplier_id='$supplier_id',supplier_name='$supplier_name',plate_number='$plate_number',wp_grade='$wp_grade',corrected_weight='$weight',unit_cost='$new_price',paper_buying='$paper_buying',branch='$branch' WHERE trans_id='$tid' and detail_id='$did' and branch='$branch' ") or die(mysql_error())) {

					$num=1;					
				}
			}
        }else {
            die("level 3 error! tid: $tid");
        }
