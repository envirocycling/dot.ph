    ...
    <div class="box round first grid">
        <h2><?php 
            ...
            if(strtoupper($filtering_branch)!='') {
                if($filtering_grade!='') {
                    echo "<h2>".strtoupper($filtering_branch)." Suppliers Quota Monitoring for the period <u>$start_date to $breaker_date</u> in MT on ".$filtering_grade."</h2>";
                }else {
                    echo "<h2>".strtoupper($filtering_branch)." Suppliers Quota Monitoring for the period <u>$start_date to $breaker_date</u>  in MT on all grades</h2>";
                }
            }else {
                if($filtering_grade!='') {
                    echo "<h2>CONSOLIDATED Suppliers Quota Monitoring for the period <u>$start_date to $breaker_date</u> in MT on wp grade: ".$filtering_grade."</h2>";
                }else {
                    echo "<h2>CONSOLIDATED Suppliers Quota Monitoring for the period <u>$start_date to $breaker_date</u> in MT on all grades</h2>";
                }
            }
            ?>
...
//////////////////////////////////
                    ...
                    foreach($months_array as $months) {
                        ...
                        if(!empty($deliveries_per_month[$key_finder2])) {
                            echo "<td>".round($deliveries_per_month[$key_finder2],2)."</button></a></td>";
                        }
                        else {
                            echo "<td >-</td>";
                        }
