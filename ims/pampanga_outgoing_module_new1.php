...
foreach ($receiving_details as $var) {
	...
	$delete=$receiving_detailslvl2[9];
	
	if($delete == 1){
		mysql_query("DELETE from outgoing WHERE branch='$branch' and trans_id='$id'") or die(mysql_error());
	}
    ...
    if(mysql_query("INSERT INTO outgoing(trans_id,detail_id,str,date,trucking,plate_number,wp_grade,weight,branch)
                                VALUES('$id','$dt_id','$str','$date','$supplier','$plate_no','$wp_grade','$weight','$branch')")or die (mysql_error())) {
     
	 $num=1;
    }
	}else if(mysql_num_rows($check) > 0){
		 if(mysql_query("UPDATE outgoing SET trans_id='$id',detail_id='$dt_id',str='$str',date='$date',trucking='$supplier',plate_number='$plate_no',wp_grade='$wp_grade',weight='$weight',branch='$branch' WHERE trans_id='$id' And detail_id='$dt_id'")or die (mysql_error())) {
	 $num=1;
    }
	}
...
?><script>
setTimeout(function () {
window.top.location.href = "http://10.151.16.231/export_receiving.php?strno=<?php echo $str.'& tid='.$id.'& update=outgoing';?>";
}, 15000);
</script>
<?php
}else if($num == 0){
