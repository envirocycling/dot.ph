    ...
    <div class="grid_10">
        <div class="box round first grid">
            <table class="data display datatable" id="example">
                ...
                if($from=='') {
                    echo "<h2> ".$branch." Inventory Analysis as of : <u><b><i>".$ngayon."</i></b></u></h2>";
                }else {


                    echo "<h2> ".$branch." Inventory Analysis as of: <u><b><i>". date("F d, Y", strtotime($ngayon))
                            ."</i></b></u></h2>";
                }
                $total_beg=0;
                $total_receive=0;
                ...
                foreach($wp_array as $wp_grade) {
                    ...
                    if($wp_grade!='LCWL' && $wp_grade!='CHIPBOARD'  && $wp_grade!='LCCB') {
                            $query3="SELECT wp_grade,sum(bale_weight) from bales where date >='$from' and date<='$to' and branch like '%$branch%'  and wp_grade='$wp_grade2'   and str_no!='VOID'  and  (out_date > '$to' or out_date < '$from' or out_date=''  )   and (date_rebaled > '$to' or date_rebaled < '$from' or date_rebaled='')   group by wp_grade";
                    }else {
                        if($wp_grade=='CHIPBOARD'  || $wp_grade=='LCCB') {
                            $query3="SELECT wp_grade,sum(bale_weight) from bales where date >='$from' and date<='$to' and  branch like '%$branch%'  and (wp_grade='cb' or wp_grade='chipboard')   and str_no!='VOID'  and  ((out_date > '$to' or out_date < '$from' or out_date='' ))   and (date_rebaled > '$to' or date_rebaled < '$from' or date_rebaled='')    group by wp_grade";
                        }else {
                            $query3="SELECT wp_grade,sum(bale_weight) from bales where date >='$from' and date<='$to' and  branch like '%$branch%'  and wp_grade='LCWL'   and str_no!='VOID'  and  ((out_date > '$to' or out_date < '$from' or out_date='' ))   and (date_rebaled > '$to' or date_rebaled < '$from' or date_rebaled='')   group by wp_grade";
                        }
                    }
                    ...
                    */
                    ...
                    if($wp_grade!='LCWL' && $wp_grade!='CHIPBOARD'  && $wp_grade!='LCCB' && $wp_grade !='lccb' && $wp_grade !='CB') {
                        $query6="SELECT wp_grade,count(wp_grade) from bales where wp_grade='$wp_grade2' and  date >='$from' and date<='$to' and ((out_date > '$to' or out_date < '$from' or out_date='' or str_no='0'))    and branch like '%$branch%'  and status !='rebaled'  group by wp_grade";
                    }else {
                        if($wp_grade=='CHIPBOARD'  || $wp_grade=='LCCB' || $wp_grade =='chipboard' || $wp_grade =='cb') {
                            $query6="SELECT wp_grade,count(wp_grade) from bales where (wp_grade='cb' or wp_grade='chipboard' or wp_grade ='CB' or wp_grade='CHIPBOARD')  and  date >='$from' and date<='$to' and ((out_date > '$to' or out_date < '$from' or out_date='' or str_no='0'))  and str_no!='VOID' and  branch like '%$branch%'  and status !='rebaled'  group by wp_grade";
                        }else {
                            $query6="SELECT wp_grade,count(wp_grade) from bales where wp_grade='LCWL' and  date >='$from' and date<='$to' and ((out_date > '$to' or out_date < '$from' or out_date='' or str_no='0'))    and str_no!='VOID'  and  branch like '%$branch%'  and status !='rebaled'  group by wp_grade";
                        }
                    }
