...
if (isset($_POST['submit'])) {
    ...
    mysql_query("INSERT INTO comment_mm (mm_id, comment, date, user_id) VALUES('" . $_GET['mm_id'] . "', '" . mysql_real_escape_string($_POST['comment']) . "', '" . date('Y/m/d H:i') . "', '" . $_SESSION['emp_num'] . "')") or die(mysql_error());

    $to = '';
    $sql_group = mysql_query("SELECT DISTINCT(user_id) from comment_leave WHERE mm_id='" . $_GET['mm_id'] . "'");
    while ($row_group = mysql_fetch_array($sql_group)) {
        $sql_emails = mysql_query("SELECT * from email WHERE emp_num='" . $row_group['user_id'] . "' and emp_num!='" . $row_mm['verified_id'] . "' and emp_num!='" . $row_mm['approved_id'] . "' and department!='agency' and status=''") or die(mysql_error());
        while ($row_email = mysql_fetch_array($sql_emails)) {
            $to .= $row_email['email'] . ',';
        }
    }

    $sql_emailsTO1 = mysql_query("SELECT * from email WHERE emp_num='" . $row_mm['verified_id'] . "' and department!='agency' and status=''") or die(mysql_error());
    if (mysql_num_rows($sql_emailsTO1) > 0) {
        $row_emailTO1 = mysql_fetch_array($sql_emailsTO1);
        $to .= $row_emailTO1['email'].',';
    }

    $sql_emailsTO2 = mysql_query("SELECT * from email WHERE emp_num='" . $row_mm['approved_id'] . "' and department!='agency' and status=''") or die(mysql_error());
                    ...
                    <div class="col-md-6">
                        <div class="panel panel-primary">
                            <div class="panel-heading">
                                ...
                                <?php
                                $sql_comment = mysql_query("SELECT * from comment_mm WHERE mm_id='" . $_GET['mm_id'] . "' ORDER BY date Asc") or die(mysql_error());
                                ?>
