...
if (isset($_POST['submit'])) {
    ...
    if ($_POST['cost'] > 0) {
        $content = 'Agency was already take an action and this delinquency has cost involved.';
    } else {
        $content = 'Agency was already take an action.';
    }
    ...
    mysql_query("UPDATE delinquency SET action = '" . mysql_real_escape_string($_POST['action']) . "', status='action taken', implementation_status='pending to hr',action_date='" . date('Y-m-d') . "', action_cost='" . $_POST['cost'] . "', attachment='" . $_GET['d_id'] . '-' . basename($_FILES["file"]["name"]) . "' WHERE d_id = '" . $_GET['d_id'] . "' ") or die(mysql_error());
        ...
        <form action="" method="post" enctype="multipart/form-data">
            <div class="container">
                ...
                <!--                        Status: <select name="status" style="width:115px; height: 30px; margin-top: 10px;" required>
                                        ...
                                        Attach File (NTE or Cost quotation or Etc.): <input type="file" style=" height: 30px; margin-top: 10px;" name="file"/> <a href="../register_nte.php?active=register&d_id=<?php echo $row_pr['d_id'] . '&emp_num=' . $row_pr['emp_num']; ?>" target="_top"><h4><b>Click here to create Notice to Explain</b></h4></a><br/>
