...
if (@$_GET['status'] == 'reliever') {
    $url = 'view_leave_reliever.php?status=reliever';
    $reliever_stat = 1;
} else {
    $url = 'view_leave.php?status=request';
    $reliever_stat = 0;
}
if (isset($_POST['approve'])) {
    ...
    if (mysql_query("UPDATE leaves SET status = '$status', $reliever_no='approved' WHERE leave_id = '" . $_GET['leave_id'] . "'") or die(mysql_error())) {
        echo '<script>
                window.top.location.href="' . $url . '&active=view&page=request&http=201";
        </script>';
    } else {
        echo '<script>
                window.top.location.href="' . $url . '&active=view&page=request&http=400";
        </script>';
    }
...
if (isset($_POST['cancel'])) {
    ...
    if ($action != 'Cancel Request') {
        if ($action == 'Approve') {
            $action_sup = strtolower($action) . 'd';
            $stat = 'pending to gm';
        } else {
            $action_sup = strtolower($action) . 'd';
            $stat = strtolower($action) . 'd';
        }
        if (mysql_query("UPDATE leaves SET supervisor_status='$action_sup', status = '$stat', cancelled_user_type='" . $_SESSION['user_type'] . "', cancelled_date = '" . date('Y/m/d H:i') . "' WHERE leave_id = '" . $_POST['emp_num'] . "'") or die(mysql_error())) {
            echo '<script>
                window.top.location.href="view_leave.php?status=request&active=view&http=201";
        </script>';
        }
    } else if (mysql_query("UPDATE leaves SET supervisor_status='$sup', head_status='$head', status = 'cancelled', cancelled_user_type='" . $_SESSION['user_type'] . "', cancelled_date = '" . date('Y/m/d H:i') . "' WHERE leave_id = '" . $_POST['emp_num'] . "'") or die(mysql_error())) {
        echo '<script>
                window.top.location.href="view_leave.php?status=request&active=view&http=201";
        </script>';
    }
...
$can_mes = 'This request has been cancelled';
//chk who's cancelled end
//chk who's cancelled start
$can_mes1 = 'This request has been disapproved';
//chk who's cancelled end
...
if (strpos($row_leave_form['status'], 'pending') !== false && $reliever_stat == 0) {
    echo "<form method='post' onsubmit='return confirm(\"Do you want to proceed?\");'>";
    ...
    if ($row_leave_form['emp_num'] == $row_user['emp_num']) {
        ...
        <?php
    } else if ($row_leave_form['supervisor_status'] == 'pending' && $row_leave_form['status'] == 'pending to supervisor' && $row_leave_form['supervisor_id'] == $row_user['emp_num']) {
        echo '<input type="submit" class="btn btn-primary" value="Approve" name="cancel">';
        echo ' | <input type="submit" class="btn btn-danger" value="Disapprove" name="cancel">';
    }
}
