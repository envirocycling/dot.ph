...
if(mysql_query("INSERT INTO personnel_requisition (date_requested, branch_id, date_needed, no_needed, job_title, employment_status, company_id, reason, education_req, special_skills, attitudinal_req, other_skills, recommendation, salary, status, prepared_id, gm_id, hr_id, gm_status, hr_status)
        VALUES ('$date_requested', '$branch', '$date_needed', '$no_needed', '$job_title', '$status', '$agency', '$reason', '$ed_requirements', '$special_skills', '$attitudinal_req', '$other_skills', '$recommendation', '$salary','pending', '$prepared_by', '$gm_empnum', '$hr_empnum', 'pending', 'pending')") or die(mysql_error())){
    ...
    echo '<script>
            location.replace("../view_pr.php?status=active&active=view&http=201");
    </script>';
}else{
    echo '<script>
