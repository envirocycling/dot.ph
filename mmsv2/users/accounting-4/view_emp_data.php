...
if (isset($_POST['update'])) {
    ...
    if (@$uploadOk == 0) {
        ...
        if (move_uploaded_file(@$_FILES["fileToUpload"]["tmp_name"], $target_dir . $new_filename)) {
            ...
            ?>
            <script>
                window.top.location.href = "view_employee.php?status=active&active=view&http=200";
            </script>		
            <?php
        } else {
            echo '<script>
...
if (@$emp_deactivated == 1) {
    ...
    echo '<td><b>Date Separated: <i>' . date('F d, Y', strtotime($row_emp_data['date_separated'])); if($row_company['type'] == '1'  && mysql_num_rows($sql_clearance) > 0){echo '<br><a href="../../forms/clearance.php?emp_num='.$row_emp_data['emp_num'].'" target="_blank">View clearance form</a></i></b>';} echo'</td>
