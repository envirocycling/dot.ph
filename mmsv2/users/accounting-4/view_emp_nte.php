...
<script>
    $(document).ready(function () {
        ...
        $('.buttons').click(function () {
            ...
            window.top.location.href = 'view_emp_ntePrint.php?nte_id=' + _thisId;
        });
    });
</script>
<?php
...
if (isset($_POST['submit'])) {
    ...
    $actions = mysql_real_escape_string($row_del['action'] . '. Attachment was already uploaded.');
    mysql_query("UPDATE delinquency SET implementation_status='pending to hr', action='$actions' WHERE nte='" . $row_nte['nte_id'] . "'");
    ...
    if (mysql_num_rows($sql_agency) > 0) {
        $cc = "CC: ";
        $sql_emailAgency = mysql_query("SELECT * from email WHERE department = 'agency' and status='' and emp_num='" . $row_emp['company_id'] . "'") or die(mysql_error());
        while ($row_emailAgency = mysql_fetch_array($sql_emailAgency)) {
            $cc .= $row_emailAgency['email'] . ',';
        }
    }

    $sql_bh = mysql_query("SELECT * from users WHERE status='' and branch_id LIKE '%(" . $row_emp['branch_id'] . ")%' and user_type='3'") or die(mysql_error());
...
if ($row_nte['supervisor_num'] > 0) {
    ...
    $supSig = '<font color="blue"><i>Signed</i></font><br>';
}
...
?>
<center>
    <img src="../../images/button/print_icon.png" width="90" height="90" title="Print" class="buttons" id="<?php echo $row_nte['nte_id']; ?>">
    <table width="85%">
        <tr>
        ...
        <tr>
            <td colspan="2"><?php echo $supSig . '<br/><span class="txt3">' . $employee_fullname_sub . '</span><br><span class="txt4">' . $rowPos . '</span><br /><br /><br /><br />'; ?></td>
            <td></td>
        </tr>
        <tr>
