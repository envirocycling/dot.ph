...
if (isset($_POST['submit'])) {
    if (mysql_query("UPDATE delinquency SET implementation_status='cleared', status='pending to hr' WHERE d_id = '" . $_GET['d_id'] . "'") or die(mysql_error())) {
        ...
        $subject = "DELINQUENCY REPORT OF " . $fullname;
        $message = "Good day Ma'am/Sir \r\n Amount charge to " . $fullname . " was already deducted/collected. This request is now pending to efi hr.\r\n\n Please Login here http://mmsv2.efi.net.ph/ to do more action.\r\n\n Thank you. \r\n\n Note: This is a system generated email, do not reply to this email. Use Google Chrome or Mozilla Firefox to view it properly.";
        $headers = "From: envirocyclingfiberincorporated@efi.net.ph" . "\r\n" . $cc;
        mail($to, $subject, $message, $headers);
...
//                    echo '<div class="action">Action Date: ' . date('Y/m/d', strtotime($row_del['action_date']));
//                    if (!empty($row_del['attachment'])) {
//                        echo '<br/>Attachment: <a href="../../attachment/delinquency/' . $row_del['attachment'] . '" target="_blank"><i>' . $row_del['attachment'] . '</i></a>';
//                    }
...
//                
                ...
                if (!empty($exAtt[1])) {
                    echo '<br/><div class="action">Attachment: <a href="../../attachment/delinquency/' . $row_del['d_id'] . '-' . $row_del['attachment'] . '" target="_blank"><i>' . $row_del['attachment'] . '</i></a></div>';
                }
        ...
        <tr>
            <td colspan="4" ><br>
                ...
                if (!empty($row_del['acknowledgment'])) {
                    if ($row_del['implementation_status'] == 'pending to accounting') {
                        echo'&nbsp;&nbsp;&nbsp;&nbsp;<form method="post" onsubmit="return confirm(\'Do you want to proceed?\');"><input class="btn btn-primary" type="submit" name="submit" value="Mark as Cleared"></form>';
                    }
                }
                ?>
