...
?>
    <center>
                        <table width="85%">
                            ...
                            <tr>
                                <td colspan="2"><br><br>Disciplinary action (To be filled out by company): 
                                    <?php
                                    $sql_company = mysql_query("SELECT * from company WHERE company_id='".$row_del['company_id']."' and type=1") or die(mysql_error());
                                        if($row_del['status'] == 'pending' && mysql_num_rows($sql_company) > 0){ echo '<button class="btn btn-primary" id="record">Record Action Taken</button>';}
                                    ?>
                                </td>
                            </tr>
                            <?php 
                                ...
                                <td colspan="4" id='val3'>
                                    <br><br><?php echo 'Action Date: '.date('Y/m/d', strtotime($row_del['action_date'])).'<br/>Status: '.ucwords($row_del['status']);
                                            ...
                                            if(!empty($row_del['acknowledgment'])){
                                               ...
                                               if($row_del['implementation_status'] == 'cleared' && $row_del['status'] == 'not clear'){echo'&nbsp;&nbsp;&nbsp;&nbsp;<form method="post" onsubmit="return confirm(\'Do you want to proceed?\');"><input class="btn btn-primary" type="submit" name="submit" value="Mark as Cleared"></form>';}
                                            }
