...
<?php
    ...
    if(isset($_POST['noted'])){
        ...
        if(mysql_query("UPDATE personnel_requisition SET hr_status='".$stat_hr."', status='".$sat."', hr_serve_date='".date('Y-m-d')."', emp_num='$served_emp' WHERE pr_id = '".$_GET['pr_id']."'") or die(mysql_error())){
            echo '<script>
                    window.top.location.href="view_pr.php?status=active&active=view&http=200";
                </script>';
        }else{
