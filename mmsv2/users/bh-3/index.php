                    ...
                    <div class="main">
                        ...
                        <div class="main_wrapper">
                            ...
                            <div id="event_frames">
                                <br />
                                <?php
                                $sql_noti_leave = mysql_query("SELECT * from leaves WHERE supervisor_status='pending' and status='pending to supervisor' and emp_num!='" . $row_user['emp_num'] . "' and supervisor_id='" . $row_user['emp_num'] . "'") or die(mysql_error());
                                $row_noti_leave = mysql_num_rows($sql_noti_leave);

                                if ($row_noti_leave > 0) {
                                    $pend = 0;
                                    while ($row_notit_leave = mysql_fetch_array($sql_noti_leave)) {
                                        if (strpos($row_user['branch_id'], '(' . $row_notit_leave['branch_id'] . ')') !== false) {
                                            $pend++;
                                        }
                                    }

                                    if ($pend > 0) {
                                        echo '<a href="view_leave.php?status=request&active=view">You have <font color="black">' . $pend . ' leave request to approve</font>.</a><hr>';
                                    }
                                    ...
                                    $row_noti_leave = mysql_num_rows($sql_noti_leave);
                                    if ($row_noti_leave > 0) {
                                        echo '<a href="view_leave_reliever.php?active=view&page=requestt">You have <font color="black">' . $row_noti_leave . ' pending reliever request</font>.</a><hr>';
                                    }
                                }
//echo "SELECT * from delinquency WHERE implementation_status='pending to supervisor' and submitted_by = '".$_SESSION['user_id']."'<br>";
                                $sql_noti_del = mysql_query("SELECT * from delinquency WHERE implementation_status='pending to supervisor' and submitted_by = '" . $_SESSION['user_id'] . "'") or die(mysql_error());
                                $row_noti_del = mysql_num_rows($sql_noti_del);

                                if ($row_noti_del > 0) {
                                    while ($row_notit_del = mysql_fetch_array($sql_noti_del)) {
                                        $sql_company = mysql_query("SELECT * from company WHERE company_id = '" . $row_notit_del['company_id'] . "' and type=1");
                                ...
                                $sql_noti_ia = mysql_query("SELECT * from incident_accident WHERE cost > 0 and form!='' and prepared_num = '" . $_SESSION['emp_num'] . "'") or die(mysql_error());
...
//                                }
                                if (@$del_pending > 0) {
                                    echo '<a href="view_delinquency.php?status=active&active=view&noti=1">You have <font color="black">' . $row_noti_ia . ' pending to upload authority to deduct or charge slip </font> incident/accident report.</a><hr>';
                                }

                                $sql_noti_sepa = mysql_query("SELECT * from form_clearance WHERE supervisor_status!='cleared' and supervisor_num='" . $_SESSION['emp_num'] . "'") or die(mysql_error());
