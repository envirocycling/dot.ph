                ...
                <div class="container">
                    ...
                    <div class="title">
                        ...
                        if(isset($_POST['filter'])){
                                if(@$_POST['status'] == 'all'){
                                    ...
                                    $status_filter = 'pending to supervisor';  
                                }else{
                                    $status_filter = @$_POST['status'];
                                }
                                    $sql_leaves = mysql_query("SELECT * from leaves WHERE (reliever1_status LIKE '$status_filter%' or reliever2_status LIKE '$status_filter%') and emp_num!='".$row_user['emp_num']."' and (reliever1_id='".$row_emp['emp_num']."' or reliever2_id='".$row_emp['emp_num']."') and date_submitted >= '".$_POST['from']."' and date_submitted <= '".$_POST['to']."' and branch_id = '".$row_emp['branch_id']."' ORDER by date_submitted Asc") or die(mysql_error());
                                $from = $_POST['from'];
                                 $to = $_POST['to'];
                            }else{
                                 $sql_leaves = mysql_query("SELECT * from leaves WHERE status LIKE  'pending to reliever' and emp_num!='".$row_user['emp_num']."' and (reliever1_id='".$row_emp['emp_num']."' or reliever2_id='".$row_emp['emp_num']."') and (reliever1_status='pending' or reliever1_status='pending') and date_submitted LIKE '".date('Y-m')."%' and branch_id = '".$row_emp['branch_id']."' ORDER by date_submitted Asc") or die(mysql_error());
                                 $from = date('Y/m');
                                 $from = $from.'/01'; 
                                $to = date('Y/m/t');                                
                            }
                    ...
                    <div class="main">
                        <br><br><center>
                            <table width='100%'>
                                <tr>
                                    <td class="header1" colspan="4"><center><?php echo '<font color="red"><b>Reliever Request</font></b>'?> Leaves<br></center></td>
                                </tr>
                                <tr>
                                    <td colspan="4"><hr></td>
                                </tr>
                                                ...
                                                while($row_leaves = mysql_fetch_array($sql_leaves)){
                                                    ...
                                                    if (strpos($row_user['branch_id'], '('.$row_emp_branch['branch_id'].')') !== false) {
                                                        echo '<tr>
                                                                ...
                                                                <td class="data"><input type="image" data-jAlert data-title="Employee Leave Form" data-iframe="view_emp_leave.php?leave_id='.$row_leaves['leave_id'].'&status=reliever" title="View" data-fullscreen="true" src="../../images/button/view_icon.png" width="40" height="40">';
