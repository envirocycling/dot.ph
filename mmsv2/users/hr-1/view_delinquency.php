                ...
                <div class="container">
                    ...
                    <div class="title">
                        ...
                        if (isset($_POST['filter'])) {
                            ...
                            $sql_del = mysql_query("SELECT * from delinquency WHERE status LIKE '$status_filter%' and date_committed >= '" . $_POST['from'] . "' and date_committed <= '" . $_POST['to'] . "'") or die(mysql_error());
                            $from = $_POST['from'];
                            $to = $_POST['to'];
                        } else {
                            if(@$_GET['noti'] > 0) {
                                $sql_del = mysql_query("SELECT * from delinquency WHERE ((status='pending' and implementation_status='') or implementation_status='pending to hr' or status='pending to hr')") or die(mysql_error());
                            } else {
                                $sql_del = mysql_query("SELECT * from delinquency WHERE (status LIKE '%pending%' or implementation_status LIKE '%pending%')") or die(mysql_error());
                            }
                            $from = date('Y/m');
                            $from = $from . '/01';
                            $to = date('Y/m/t');
                                        ...
                                        <table  class="data display datatable">
                                            ...
                                            while ($row_del = mysql_fetch_array($sql_del)) {
                                                ...
                                                echo '<tr class="data2">
                                                            ...
                                                            <td><input type="image" data-jAlert data-title="Employee Delinquency Form" data-iframe="view_emp_delinquency.php?d_id=' . $row_del['d_id'] . '" title="View" data-fullscreen="true" src="../../images/button/view_icon.png" width="40" height="40"> | <img src="../../images/button/print_icon.png" width="40" height="40" title="Print" class="buttons" id="' . $row_del['d_id'] . '">';
                                                echo '</td>
                                                    </tr>';
                                                ?>
                                                <?php
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                ...
                $('.buttons').click(function () {
                    ...
                    window.open('view_emp_delinquencyPrint.php?d_id=' + _thisId);
                });
            });
