...
if (isset($_POST['validate'])) {
    mysql_query("UPDATE leaves SET validated='1', date_affected1='" . date('Y-m-d H:i', strtotime($_POST['date_affected1'])) . "', date_affected2='" . date('Y-m-d H:i', strtotime($_POST['date_affected2'])) . "', no_days='" . $_POST['no_days'] . "' WHERE leave_id = '" . $_POST['emp_num'] . "'") or die(mysql_error());
    echo '<script>
            window.top.location.href="view_leave.php?status=for%20validation&active=view&http=200";
        </script>';
} else if (isset($_POST['cancel'])) {
    mysql_query("UPDATE leaves SET status = 'cancelled', cancelled_user_type='" . $_SESSION['user_id'] . "', cancelled_date = '" . date('Y/m/d H:i') . "' WHERE leave_id = '" . $_POST['emp_num'] . "'") or die(mysql_error());
} else if (isset($_POST['cancel2'])) {
    $action = $_POST['cancel2'];
    if ($action == 'Cancel Request') {
        mysql_query("UPDATE leaves SET status = 'cancelled', cancelled_user_type='" . $_SESSION['user_type'] . "', cancelled_date = '" . date('Y/m/d H:i') . "' WHERE leave_id = '" . $_POST['emp_num'] . "'") or die(mysql_error());
    } else if ($action == 'Approve') {
        mysql_query("UPDATE leaves SET 	supervisor_status = 'approved',	status='pending to gm', supervisor_date = '" . date('Y/m/d H:i') . "' WHERE leave_id = '" . $_POST['emp_num'] . "'") or die(mysql_error());
    } else if ($action == 'Disapprove') {
        mysql_query("UPDATE leaves SET status = 'disapproved', supervisor_status = 'disapproved', cancelled_user_type='" . $_SESSION['user_id'] . "', supervisor_date = '" . date('Y/m/d H:i') . "' WHERE leave_id = '" . $_POST['emp_num'] . "'") or die(mysql_error());
    }
} else if (isset($_POST['cancel3'])) {
    $action = $_POST['cancel3'];
    $reliever_no = $_POST['reliever_no'];
    if ($action == 'Cancel Request') {
        mysql_query("UPDATE leaves SET status = 'cancelled', cancelled_user_type='" . $_SESSION['user_type'] . "', cancelled_date = '" . date('Y/m/d H:i') . "' WHERE leave_id = '" . $_POST['emp_num'] . "'") or die(mysql_error());
    } else if ($action == 'Approve') {
        mysql_query("UPDATE leaves SET 	supervisor_status = 'approved', $reliever_no='approved', status='pending to gm', supervisor_date = '" . date('Y/m/d H:i') . "' WHERE leave_id = '" . $_POST['emp_num'] . "'") or die(mysql_error());
    } else if ($action == 'Disapprove') {
        mysql_query("UPDATE leaves SET status = 'disapproved', supervisor_status = 'disapproved', $reliever_no='disapproved', cancelled_user_type='" . $_SESSION['user_id'] . "', supervisor_date = '" . date('Y/m/d H:i') . "' WHERE leave_id = '" . $_POST['emp_num'] . "'") or die(mysql_error());
    }
...
$sql_emp = mysql_query("SELECT * from employees WHERE emp_num = '" . $row_leave_form['emp_num'] . "'");
if(mysql_num_rows($sql_emp) == 0){
    $sql_emp = mysql_query("SELECT * from employees_deactivated WHERE emp_num = '" . $row_leave_form['emp_num'] . "'");
}
...
if ($row_leave_form['reliever1_id'] > 0) {
    ...
    if ($row_leave_form['reliever1_status'] == 'pending' && $row_leave_form['reliever1_id'] == $row_user['emp_num']) {
        echo "<form method='post' onsubmit='return confirm(\"Do you want to proceed?\");'>";
...
if ($row_leave_form['reliever2_id'] > 0) {
    ...
    if ($row_leave_form['reliever2_status'] == 'pending' && $row_leave_form['reliever2_id'] == $row_user['emp_num']) {
        echo "<form method='post' onsubmit='return confirm(\"Do you want to proceed?\");'>";
...
if (strpos($row_leave_form['status'], 'pending') !== false) {
    ...
    if ($row_leave_form['emp_num'] == $row_user['emp_num']) {
        ?>
        <input type="submit" class="btn btn-danger" value="Cancel Request" name="cancel">
        <?php
    } else if ($row_leave_form['supervisor_status'] == 'pending' && $row_leave_form['supervisor_id'] == $row_user['emp_num'] && $row_leave_form['reliever2_id'] == $row_user['emp_num']) {
        echo '<input type="hidden" name="reliever_no" value="reliever2_status">';
        echo '<input type="submit" class="btn btn-primary" value="Approve" name="cancel3">';
        echo ' | <input type="submit" class="btn btn-danger" value="Disapprove" name="cancel3">';
    } else if ($row_leave_form['supervisor_status'] == 'pending' && $row_leave_form['supervisor_id'] == $row_user['emp_num'] && $row_leave_form['reliever1_id'] == $row_user['emp_num']) {
        echo '<input type="hidden" name="reliever_no" value="reliever1_status">';
        echo '<input type="submit" class="btn btn-primary" value="Approve" name="cancel3">';
        echo ' | <input type="submit" class="btn btn-danger" value="Disapprove" name="cancel3">';
    } else if ($row_leave_form['supervisor_id'] == $row_user['emp_num'] && $row_leave_form['status'] == 'pending to supervisor') {
        echo '<input type="submit" class="btn btn-primary" value="Approve" name="cancel2">';
        echo ' | <input type="submit" class="btn btn-danger" value="Disapprove" name="cancel2">';
    }
}
...
if ($row_leave_form['validated'] == 0 && $row_leave_form['status'] != 'cancelled' && $row_leave_form['supervisor_status'] == 'approved') {
    echo '<input type="hidden" value="' . $_GET['leave_id'] . '" name="emp_num">
                        <input type="submit" class="btn btn-primary" value="Validate" name="validate">&nbsp;&nbsp;&nbsp;';
    echo "</form>";
}
if (($row_leave_form['validated'] == 1 || $row_leave_form['validated'] == 0) && $row_leave_form['status'] != 'cancelled') {
    ?>
    <input type="hidden" value="<?php echo $_GET['leave_id']; ?>" name="emp_num">
    <input type="submit" class="btn btn-danger" value="Cancel Request" name="cancel">
    <?php
}
?>
