...
//            });
        $('#datetimepicker').change(function(){
            var sl_date = $(this).val();
            var newdate = new Date(sl_date);
    		//var y = newdate.getFullYear();
		var mm =  new Date(newdate.setMonth(newdate.getMonth() - 2));
                var samp = mm.getMonth();
//                var someFormattedDate = samp + '-';
                ...
                <div class="container">
                    ...
                    <div class="title">
                        ...
                        if(isset($_POST['filter'])){
                                ...
                                $q_to = $_POST['to'].' 23:59:59';
                                if($_GET['status'] == 'sent'){
                                    $sql_leaves = mysql_query("SELECT * from leaves WHERE status LIKE '$status_filter%' and emp_num='".$row_user['emp_num']."' and date_affected1 >= '".$_POST['from']."' and date_affected1 <= '$q_to' ORDER by date_affected1 Asc") or die(mysql_error());
                                }else if($_GET['status'] == 'for validation'){
                                    $sql_leaves = mysql_query("SELECT * from leaves WHERE status LIKE '$status_filter%' and date_affected1 >= '".$_POST['from']."' and date_affected1 <= '$q_to' and supervisor_status='approved' and validated=0 and status != 'cancelled' ORDER by date_affected1 Asc") or die(mysql_error());
                                }else{
                                    $sql_leaves = mysql_query("SELECT * from leaves WHERE status LIKE '$status_filter%' and emp_num!='".$row_user['emp_num']."' and date_affected1 >= '".$_POST['from']."' and date_affected1 <= '$q_to' ORDER by date_affected1 Asc") or die(mysql_error());
                                }
                                $from = $_POST['from'];
                                ...
                                if($_GET['status'] == 'sent'){
                                 $sql_leaves = mysql_query("SELECT * from leaves WHERE status LIKE  'pending%' and emp_num='".$row_user['emp_num']."' and date_affected1 LIKE '".date('Y')."%'  ORDER by date_affected1 Asc") or die(mysql_error());
                                }else if($_GET['status'] == 'for validation'){
                                    $sql_leaves = mysql_query("SELECT * from leaves WHERE  date_affected1 LIKE '".date('Y')."%' and validated=0 and supervisor_status='approved' and status!='cancelled' ORDER by date_affected1 Asc") or die(mysql_error());
                                }else if(@$_GET['page'] == 'request'){
                                 $sql_leaves = mysql_query("SELECT * from leaves WHERE status LIKE  'pending%' and emp_num!='".$row_user['emp_num']."' and supervisor_id = '".$row_user['emp_num']."'  ORDER by date_affected1 Asc") or die(mysql_error());
                                }else{
                                 $sql_leaves = mysql_query("SELECT * from leaves WHERE status LIKE  'pending%' and emp_num!='".$row_user['emp_num']."' and date_affected1 LIKE '".date('Y')."%'  ORDER by date_affected1 Asc") or die(mysql_error());
                                }
                                $from = date('Y/m');
                                ...
                                <tr>
                                    <td>
                                        ...
                                        <table  class="data display datatable">
                                            <thead>
                                                <tr class="data">
                                                    <th class="data">Date Affected</th>
                                            ...
                                            <?php
                                                ...
                                                while($row_leaves = mysql_fetch_array($sql_leaves)){
                                                    ...
                                                    if(mysql_num_rows($sql_emp) == 0){
                                                        $sql_emp = mysql_query("SELECT * from employees_deactivated WHERE emp_num = '".$row_leaves['emp_num']."'") or die(mysql_error());
                                                    }
                                                    ...
                                                    $num = 1;
                                                    echo '<tr class="data2">
                                                            <td>'.date('Y/m/d', strtotime($row_leaves['date_affected1'])).'</td>
                                                            <td>'.$row_leaves['leave_id'].'</td>
                                                            <td>'.$employee_fullname.'</td>
                                                            <td>'.$row_emp_position['position'].'</td>
                                                            <td>'.$row_emp_company['name'].'</td>
                                                            <td>'.$row_emp_branch['branch_name'].'</td>
                                                            <td>'.$row_leaves['leave_type'].'</td>
                                                            <td>'.ucwords($row_leaves['status']).'</td>
                                                            <td><input type="image" data-jAlert data-title="Employee Leave Form" data-iframe="view_emp_leave.php?leave_id='.$row_leaves['leave_id'].'" title="View" data-fullscreen="true" src="../../images/button/view_icon.png" width="40" height="40">';
