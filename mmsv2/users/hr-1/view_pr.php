                        ...
                        include 'layout/menu.php';
                            if(isset($_POST['filter'])){
                                if(@$_POST['status'] == 'all'){
                                 ...
                                 $sql_pr = mysql_query("SELECT * from personnel_requisition WHERE (gm_status LIKE '$status_filter%' or hr_status LIKE '$status_filter%') and date_requested >= '".$_POST['from']."' and date_requested <= '".$_POST['to']."' ORDER by date_requested Asc") or die(mysql_error());
                                 $from = $_POST['from'];
                                 $to = $_POST['to'];
                            }else{
                                 $sql_pr = mysql_query("SELECT * from personnel_requisition WHERE status LIKE 'pending%' and hr_status='pending' and date_requested LIKE '".date('Y')."%' ORDER by date_requested Asc") or die(mysql_error());
                                 $from = date('Y/m');
                                 $from = $from.'/01'; 
                                $to = date('Y/m/t');                                
                            }
                                            ...
                                            <?php
                                                ...
                                                while($row_pr = mysql_fetch_array($sql_pr)){
                                                    ...
                                                    echo '<tr class="data2">
                                                            ...
                                                            <td>'.$hr_status.'</td>
                                                            <td>'.$served_tat.'</td>
                                                            <td><input type="image" data-jAlert data-title="Personnel Rrequest Form" data-iframe="view_emp_pr.php?pr_id='.$row_pr['pr_id'].'" data-fullscreen="true" title="View" src="../../images/button/view_icon.png" width="40" height="40"> | <img src="../../images/button/print_icon.png" width="40" height="40" title="Print" class="buttons" id="' . $row_pr['pr_id'] . '">';
