...
//                                }
                            ...
                            <div id="event_frames">
                                <br />
                                <?php
                                $sql_leave = mysql_query("SELECT * from leaves WHERE validated = 0 and (manager_status='approved' or supervisor_status='approved') and status NOT LIKE '%cancelled%'") or die(mysql_error());
                                $row_leaveNum = mysql_num_rows($sql_leave);
                                if ($row_leaveNum > 0) {
                                    while ($row_leave = mysql_fetch_array($sql_leave)) {
                                        @$leave_validate += 1;
                                    }
                                    echo '<a href="view_leave.php?status=for validation&active=view">You have <font color="black">' . @$leave_validate . ' to validate </font>leave request.</a><hr>';
                                }

                                $sql_noti_leave2 = mysql_query("SELECT * from leaves WHERE status!='cancelled' and supervisor_status='pending' and supervisor_id='" . $row_emp['emp_num'] . "'") or die(mysql_error());
                                $row_noti_leave2 = mysql_num_rows($sql_noti_leave2);
                                if ($row_noti_leave2 > 0) {
                                    echo '<a href="view_leave.php?active=view&page=request">You have <font color="black">' . $row_noti_leave2 . ' pending leave request to approve</font>.</a><hr>';
                                }

                                $sql_noti_leave = mysql_query("SELECT * from leaves WHERE status='pending to reliever' and ((reliever1_id='" . $row_emp['emp_num'] . "' && reliever1_status='pending') or (reliever2_id='" . $row_emp['emp_num'] . "' && reliever2_status='pending'))") or die(mysql_error());
                                $row_noti_leave = mysql_num_rows($sql_noti_leave);
                                if ($row_noti_leave > 0) {
                                    echo '<a href="view_leave_reliever.php?active=view&page=request">You have <font color="black">' . $row_noti_leave . ' pending reliever request</font>.</a><hr>';
                                }

                                $sql_noti_pr = mysql_query("SELECT * from personnel_requisition WHERE gm_status='approved' and hr_status='pending'") or die(mysql_error());
                                $row_noti_pr = mysql_num_rows($sql_noti_pr);
                                if ($row_noti_pr > 0) {
                                    while ($row_notit_pr = mysql_fetch_array($sql_noti_pr)) {
                                        @$hr_pending += 1;
                                    }
                                    echo '<a href="view_pr.php?status=active&active=view">You have <font color="black">' . $hr_pending . ' pending </font> personnel requisition request.</a><hr>';
                                }

                                $sql_noti_pr_not = mysql_query("SELECT * from personnel_requisition WHERE gm_status='approved' and hr_status='noted'") or die(mysql_error());
                                $row_noti_pr_not = mysql_num_rows($sql_noti_pr_not);
                                if ($row_noti_pr_not > 0) {
                                    while ($row_notit_pr_not = mysql_fetch_array($sql_noti_pr_not)) {
                                        @$hr_noted += 1;
                                    }
                                    echo '<a href="view_pr.php?status=active&active=view">You have <font color="black">' . $hr_noted . ' pending to serve </font> personnel requisition request.</a><hr>';
                                }
//                                echo "SELECT * from delinquency WHERE status='pending' or implementation_status='pending to hr'<br>";
                                $sql_noti_del = mysql_query("SELECT * from delinquency WHERE ((status='pending' and implementation_status='') or implementation_status='pending to hr' or status='pending to hr')") or die(mysql_error());
                                $row_noti_del = mysql_num_rows($sql_noti_del);
                                @$del_pending = $row_noti_del;
                                if (@$del_pending > 0) {
                                    echo '<a href="view_delinquency.php?status=active&active=view&noti=1">You have <font color="black">' . @$del_pending . ' pending to take an action </font> delinquency request.</a><hr>';
                                }

//                                $sql_noti_del2 = mysql_query("SELECT * from delinquency WHERE (implementation_status='cleared' && status='not clear')  and date_submitted < '" . date('Y-m-d') . "'") or die(mysql_error());
//                                $row_noti_del2 = mysql_num_rows($sql_noti_del2);
//
//                                if ($row_noti_del2 > 0) {
//                                    while ($row_notit_del2 = mysql_fetch_array($sql_noti_del2)) {
//                                        $sql_company = mysql_query("SELECT * from company WHERE company_id = '" . $row_notit_del2['company_id'] . "' and type=0");
...
//                                }
                                if (@$del_pending2 > 0) {
                                    echo '<a href="view_delinquency.php?status=active&active=view">You have <font color="black">' . @$del_pending2 . ' not clear to verify</font> delinquency request.</a><hr>';
                                }

                                $sql_noti_ia = mysql_query("SELECT * from incident_accident WHERE status='pending to hr'") or die(mysql_error());
                                $row_noti_ia = mysql_num_rows($sql_noti_ia);
                                if ($row_noti_ia > 0) {
                                    echo '<a href="view_ia.php?status=active&active=view&noti=1">You have <font color="black">' . @$row_noti_ia . ' pending for billing to verify </font> incident/accident report.</a><hr>';
                                }

                                $sql_noti_mm = mysql_query("SELECT * from manpower_movement WHERE status='approved by gm'") or die(mysql_error());
                                ...
                                $sql_noti_sepa = mysql_query("SELECT * from form_clearance WHERE hr_status!='cleared'") or die(mysql_error());
