...
if($action == 'edit'){
    ...
    $sql_position = mysql_query("SELECT * from positions WHERE p_id = '$position_id' and status=''") or die(mysql_error());
    ...
    $position = str_replace("ampersand","&",(mysql_real_escape_string($_POST['position'])));
    $position = str_replace("**","ñ",$position);
    $position = ucwords(str_replace("(*)","Ñ",$position));
    
    $sql_chk = mysql_query("SELECT * from positions WHERE position='$position' ") or die(mysql_error());
   if(mysql_num_rows($sql_chk) == 0){
       ...
       mysql_query("UPDATE positions SET status = ''  WHERE position = '$position' and status != ''") or die(mysql_error());
   }
}