...
if ($company_id > 0) {
    ...
    mysql_query("UPDATE slider_events SET active='" . $_POST['status'] . "' WHERE id='$e_id'") or die(mysql_error());
} else if ($a_iddel > 0) {
    ...
    $sql_ann = mysql_query("SELECT * from slider_events WHERE id='$e_iddel'");
    $row_ann = mysql_fetch_array($sql_ann);
    
    $target_dir = '../../../images/events/';
    $chk_fname = $target_dir . $row_ann['id'].'-'.$row_ann['img'];
    if (file_exists($chk_fname)) {
        unlink($chk_fname);
        mysql_query("DELETE from slider_events WHERE id='$e_iddel'") or die(mysql_error());
    }
} else if ($rank_id > 0) {
