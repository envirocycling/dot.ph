...
if(mysql_query("INSERT INTO leaves (emp_num, date_submitted, branch_id, position_id, date_affected1, date_affected2, no_days, leave_type, cs_date1, cs_date2, os_date, specify, reason, reliever1_id, reliever2_id, supervisor_id, manager_id, status, reliever1_status, reliever2_status, supervisor_status, manager_status, validated, hos_date)
        VALUES ('$emp_num', '$date_submitted', '$branch', '$position', '$date_affected1', '$date_affected2', '$no_days', '$leave_type', '$cs_date1', '$cs_date2', '$os_date', '$specify', '$reason', '$reliever1', '$reliever2', '$supervisor', '$manager', '$status', '$r1_status', '$r2_status', 'pending', 'pending', '$validate','$hos_date')") or die(mysql_error())){    
    ...
    if ($status == 'pending to supervisor') {
        ...
        $sql_emailTo = mysql_query("SELECT * from email WHERE department = 'hr' and status='' and emp_num='$supervisor'") or die(mysql_error());
        ...
        $sql_max = mysql_query("SELECT max(leave_id) as leave_id from leaves");
        $row_max = mysql_fetch_array($sql_max);
        
        $subject = "MMMS: Leave Request ofs " . $fullname.'~'.$row_max['leave_id'];
        $message = "Good day Ma'am/Sir \r\n You have leave request to approve.\r\n\n Please Login here http://mmsv2.efi.net.ph/ to do more action.\r\n\n Thank you. \r\n\n Note: This is a system generated email, do not reply to this email. Use Google Chrome or Mozilla Firefox to view it properly.";
        $headers = "From: envirocyclingfiberincorporated@efi.net.ph";
        mail($to, $subject, $message, $headers);
    }
    ...
    echo '<script>
                location.replace("../view_leave.php?status=sent&active=view&http=200");
        </script>';
}else{
    echo '<script>
