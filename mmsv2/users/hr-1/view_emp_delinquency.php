...
if (isset($_POST['submit'])) {
    if (mysql_query("UPDATE delinquency SET status='cleared' WHERE d_id = '" . $_GET['d_id'] . "'") or die(mysql_error())) {
        ...
        $sql_emailMan = mysql_query("SELECT * from email WHERE department = 'manager' and status=''") or die(mysql_error());
        $row_emailMan = mysql_fetch_array($sql_emailMan);
        $cc .= $row_emailMan['email'] . ',';
        ...
        $sql_emailTo2 = mysql_query("SELECT * from email WHERE  status='' and department='accounting'") or die(mysql_error());
        while ($row_emailTo2 = mysql_fetch_array($sql_emailTo2)) {
            $cc .= $row_emailTo2['email'] . ',';
        }

        $subject = "DELINQUENCY REPORT OF " . $fullname;
        $message = "Good day Ma'am/Sir \r\n This delinquency report is now clear and close.\r\n\n Please Login here http://mmsv2.efi.net.ph/ to do more action.\r\n\n Thank you. \r\n\n Note: This is a system generated email, do not reply to this email. Use Google Chrome or Mozilla Firefox to view it properly.";
        $headers = "From: envirocyclingfiberincorporated@efi.net.ph" . "\r\n" . $cc;
        mail($to, $subject, $message, $headers);
...
<center>
    <table width="85%">
        ...
        <tr>
            <td colspan="2"><br><br><b>Disciplinary action:</b></td>
        </tr>
        <tr>
            <td colspan="4" id='val3'>
                <?php
                $exAtt = explode('-', $row_del['attachment']);
                if (!empty($row_del['action'])) {
                    $expAttach = explode('-', $row_del['attachment']);
                    if (!empty($expAttach[1])) {
        ...
        <tr>
            <td colspan="2"><br><br><b>HR action :</b>
                <?php
                if ($row_del['implementation_status'] == 'pending to hr') {
                    echo '<button class="btn btn-primary" id="record">HR Action</button>';
                } else if (!empty($row_del['hr_action']) && $row_del['type'] == 'monetary') {
        ...
        <tr>
            <td>
                <?php
                if ($row_del['implementation_status'] == 'cleared' && $row_del['type'] == 'monetary' && $row_del['status'] == 'pending to hr') {
                    echo'&nbsp;&nbsp;&nbsp;&nbsp;<form method="post" onsubmit="return confirm(\'Do you want to proceed?\');"><input class="btn btn-primary" type="submit" name="submit" value="Mark as Cleared"></form>';
                }
                ?>
            </td>
        ...
        <tr>
            <td colspan="2"><br>
                <?php if ($row_del['status'] != 'cancelled') { ?><img src="../../images/signature/<?php echo $row_user_sub['emp_num'] . '.png'; ?>"  width="100px;"><?php
                } else {
                    echo '<b><font color="red">Cancelled</b>';
                }
                ?><!--Signature--></td>
        </tr>
        <tr>
