...
<?php
    ...
    if(isset($_POST['transferred'])) {
        ...
        if(mysql_query("UPDATE manpower_movement SET status='transferred', date_transferred='$date', date_approved='$date'  WHERE mm_id = '".$_GET['mm_id']."'") or die(mysql_error())) {

            $clas_type_ex = explode('~',$row_mm['type']);
            $type = $clas_type_ex[0];
            $type_info = $clas_type_ex[1];
            $class = $row_mmp['class'];
            $cDate = date('Y/m/d');

            if($class == 'permanent'){
                $tDate = date('Y/m/d', strtotime($row_mm['per_date']));
            }else{
                $tDate = date('Y/m/d', strtotime($row_mm['temp_date1']));
            }
            
            if($type == 'transfer'){
                mysql_query("UPDATE employees SET position_id='$type_info' WHERE emp_num='".$row_mm['emp_num']."'") or die (mysql_error());
            }else if($type == 'move'){
                mysql_query("UPDATE employees SET company_id='$type_info' WHERE emp_num='".$row_mm['emp_num']."'") or die (mysql_error());
            }else if($type == 'reassign' && $tDate <= $cDate){
                mysql_query("UPDATE employees SET branch_id='$type_info' WHERE emp_num='".$row_mm['emp_num']."'") or die (mysql_error());
            }
            echo '<script>
        ...
        if(mysql_query("UPDATE manpower_movement SET status='disapproved', status='disapproved', date_approved='$date' WHERE  mm_id = '".$_GET['mm_id']."'") or die(mysql_error())){
          echo '<script>
                window.top.location.href="view_empmovement.php?status=active&active=view&http=200";
            </script>';  
        }else{
    ...
    //Receiver - 2019/05/14 - jays
    if($row_mm['bh_id'] != 0) {
        $_bh_id = $row_mm['bh_id'];
    }else {
        $_bh_id = $row_mm['verified_id'];
    }

    $sql_emp_received = mysql_query("SELECT * from employees WHERE emp_num = '$_bh_id' ") or die(mysql_error());
    $row_emp_received = mysql_fetch_array($sql_emp_received);
        
    $_sql_emp_pos = mysql_query("SELECT * from positions WHERE p_id='".$row_emp_received['position_id']."'") or die(mysql_error());
    $_row_emp_pos = mysql_fetch_array($_sql_emp_pos);
    $chk_str_received = strlen($row_emp_received['middlename']);

    if($chk_str_received > 1){
        $str_count_received = strlen($row_emp_received['middlename']) - 1;
        $middlename_received = substr($row_emp_received['middlename'],0,-$str_count_veri);
    }else{
        $str_count_received = strlen($row_emp_received['middlename']);
        $middlename_received = $row_emp_received['middlename'];
    }

    if(empty($row_emp_received['middlename'])){
        $middlename_received = '';
    }else{
        $middlename_received = ', '.$middlename_received.'.';
    }
    $fullname_received = ucwords($row_emp_received['lastname'].', '.$row_emp_received['firstname'].$middlename_received);
    //Receiver end
    ...
    <center>
                        ...
                        <table width="1100px">
                            ...
                            <tr>
                                <td colspan="3"></td>
                                <td></td>
                                <td><center><font size="-1"><?php echo ucwords($_row_emp_pos['position']);?></font></center></td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td>
                                    <center>
                                    <?php
                                        // New - 2019/03/22 - Jayson
                                        if($row_mm['status'] == 'transferred' || $row_mm['status'] == 'received') {
                                            echo '<img src="../../images/signature/'.$row_mm['approved_id'].'.png" width="90">';
                                        }else{
                                            echo '<form method="post" onsubmit="return confirm(\'Do you want to proceed? You cannot undo this process.\');"><input type="submit" class="btn btn-success" value="Approved" name="transferred">  |  <input type="submit" class="btn btn-danger" value="disapprove" name="disapprove"></form>';
                                        }
                                    ?>
                                    </center>
                                </td>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php
                                        // New - 2019/05/14 - Jays
                                        if($row_mm['status'] == 'received') {
                                            echo '<img src="../../images/signature/'.$_bh_id.'.png" width="90">';
                                        }
                                    ?>
                                    </center>
                                </td>
                            ...
                            <tr>
                                <td colspan="3"></td>
                                <td>Received By:</td>
                                <td><center><?php echo $fullname_received;?></center></td>
                            </tr>
                            <tr>
                                <td colspan="3"></td>
                                <td></td>
                            ...
                            <?php
                                // Old
                                // if($row_mm['status'] == 'approved by gm'){
                                //     echo '<tr>';
                                //     echo '<td colspan="4"><center><form method="post" onsubmit="return confirm(\'Do you want to proceed? You cannot undo this process.\')"><input type="submit" class="btn btn-primary" value="Transferred" name="transferred"></form></center></td>';
                                //     echo '</tr>';
                                // }
                            ?>
                        </table>
