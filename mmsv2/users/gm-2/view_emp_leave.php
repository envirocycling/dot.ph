...
if (isset($_POST['approve'])) {
    $action = @$_POST['action'];
    if ($action == 'same') {
        if (mysql_query("UPDATE leaves SET status = 'approved', supervisor_status='approved', manager_status='approved', supervisor_date = '$date', manager_date='$date' WHERE leave_id = '" . $_POST['emp_num'] . "'") or die(mysql_error())) {
            echo '<script>
                    window.top.location.href="view_leave.php?status=active&active=view&http=200";
            </script>';
        } else {
            echo '<script>
                    window.top.location.href="view_leave.php?status=active&active=view&http=400";
            </script>';
        }
    } else {
        if (mysql_query("UPDATE leaves SET status = 'approved', manager_status='approved', manager_date='$date' WHERE leave_id = '" . $_POST['emp_num'] . "'") or die(mysql_error())) {
            echo '<script>
                    window.top.location.href="view_leave.php?status=active&active=view&http=200";
            </script>';
        } else {
    ...
    $action = @$_POST['action'];
    if ($action == 'same') {
        if (mysql_query("UPDATE leaves SET status = 'disapproved', supervisor_status='disapproved', manager_status='disapproved', supervisor_date = '$date', manager_date='$date' WHERE leave_id = '" . $_POST['emp_num'] . "'") or die(mysql_error())) {
            echo '<script>
                    window.top.location.href="view_leave.php?status=active&active=view&http=200";
            </script>';
        } else {
        ...
        if (mysql_query("UPDATE leaves SET status = 'disapproved', manager_status='disapproved', manager_date='$date' WHERE leave_id = '" . $_POST['emp_num'] . "'") or die(mysql_error())) {
            echo '<script>
                    window.top.location.href="view_leave.php?status=active&active=view&http=200";
            </script>';
        } else {
...
echo '</td>
                            <td class="tbl3_td2"><div id="hr_not">HR Notation (Days): <br><center><h4><i>No. of Leave Submitted</i></h4></center></div>
                                ...
                                <div class="hr_label_val">Leave Balance After This: <br><div style="width: 50%; float: left">VL: <input type="text" value="' . $num_vl . '" id="balance_vl"  name="balance_vl" style="width:40%;" readonly></div><div style="width: 50%; float: left">SL: <input type="text" value="' . $num_sl . '" id="balance_sl"  style="width:40%;" name="balance_sl" readonly></div></div>
                            </td>
                        </tr>
                   </table>';

if ($row_leave_form['manager_status'] == 'pending' && $row_leave_form['status'] == 'pending to gm') {
    echo "<form method='post' onsubmit='return confirm(\"Do you want to proceed?\");'>";
    if ($row_leave_form['supervisor_id'] == $row_leave_form['manager_id']) {
        echo '<input type="hidden" name="action" value="same">';
    }
    ?>
