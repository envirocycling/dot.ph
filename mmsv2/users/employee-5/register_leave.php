...
<html lang="en">
    ...
    //end leave balance computation
                        ...
                        $sql_leaves = mysql_query("SELECT * from leaves WHERE emp_num='".$row_emp['emp_num']."' and  status LIKE 'approved%' and date_submitted LIKE '$currentYear%'") or die(mysql_error());
...
//                        $sql_supervisor = mysql_query("SELECT * from users WHERE branch_id LIKE '%(".$row_emp['branch_id'].")%'") or die(mysql_error());
                        $sql_supervisor = mysql_query("SELECT * from users WHERE ((user_type = 1 and dep_id = 5) or user_type = 2 or user_type = 4 or (user_type = 3 and dep_id = 5)) and status =''") or die(mysql_error());
...
<div class="action_name">';
                        ...
                        $e_num = $row_emp['emp_num'];

                        if($e_num == 340 || $e_num == 36 || $e_num == 47 || $e_num == 252 || $e_num == 338 || $e_num == 364 || $e_num == 370 || $e_num == 234 || $e_num == 258 || $e_num == 312 || $e_num == 245) {

                            $accounting_visor = mysql_query("SELECT * from users WHERE username='afr' AND user_type='4' AND dep_id='4' LIMIT 1;") or die(mysql_error());
                            $res_acc_visor =  mysql_fetch_array($accounting_visor);


                            $sql_emp_sup = mysql_query("SELECT * from employees WHERE emp_num = '".$res_acc_visor['emp_num']."' ") or die(mysql_error());
                            ...
                            echo $fullname_sup .'<input type=\'hidden\' value="'.$res_acc_visor['emp_num'].'" id=\'manager_name\' name=\'supervisor\' size=25 readonly class="approver">';
