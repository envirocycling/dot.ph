                        ...
                        include 'layout/menu.php';
                            if(isset($_POST['filter'])){
                                if(@$_POST['status'] == 'all'){
                                 ...
                                 $sql_del = mysql_query("SELECT * from delinquency WHERE status LIKE '$status_filter%' and date_committed >= '".$_POST['from']."' and date_committed <= '".$_POST['to']."' and emp_num='".$_SESSION['emp_num']."'") or die(mysql_error());
                                 $from = $_POST['from'];
                                 $to = $_POST['to'];
                            }else{
                                 $sql_del = mysql_query("SELECT * from delinquency WHERE (status = 'pending' or status ='not clear') and date_committed LIKE '".date('Y-m')."%' and emp_num='".$_SESSION['emp_num']."'") or die(mysql_error());
                                 $from = date('Y/m');
                                 $from = $from.'/01'; 
                                $to = date('Y/m/t');                                
                            }
