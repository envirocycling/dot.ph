                ...
                <div class="container">
                    ...
                    <div class="title">
                        ...
                        if(isset($_POST['filter'])){
                                ...
                                if($page == 'request'){
                                    $sql_leaves = mysql_query("SELECT * from leaves WHERE (reliever1_status LIKE '%$status_filter%' or reliever2_status LIKE '%$status_filter%') and (reliever1_id='".$row_emp['emp_num']."' or reliever2_id='".$row_emp['emp_num']."') and date_submitted >= '".$_POST['from']."' and date_submitted <= '".$_POST['to']."' ORDER by date_submitted Asc") or die(mysql_error());
                                }else{
                                    $sql_leaves = mysql_query("SELECT * from leaves WHERE status LIKE '$status_filter%' and emp_num='".$row_emp['emp_num']."' and date_submitted >= '".$_POST['from']."' and date_submitted <= '".$_POST['to']."' ORDER by date_submitted Asc") or die(mysql_error());
                                }    
                                $from = $_POST['from'];
                                 $to = $_POST['to'];
                            }else{
                                if($page == 'request'){
                                    $sql_leaves = mysql_query("SELECT * from leaves WHERE status LIKE 'pending%' and (reliever1_id='".$row_emp['emp_num']."' or reliever2_id='".$row_emp['emp_num']."') and (reliever1_status='pending' or reliever2_status='pending') and date_submitted LIKE '".date('Y-m')."%' ORDER by date_submitted Asc") or die(mysql_error());
                                }else{
                                    $sql_leaves = mysql_query("SELECT * from leaves WHERE status LIKE  'pending%' and emp_num='".$row_emp['emp_num']."' and manager_status LIKE '%pending%' and date_submitted LIKE '".date('Y-m')."%'  ORDER by date_submitted Asc") or die(mysql_error());
                                }
                                 $from = date('Y/m');
                                $from = $from.'/01'; 
                                $to = date('Y/m/t');                                
                    ...
                    <div class="main">
                        <br><br><center>
                            <table width='100%'>
                                <tr>
                                    <td class="header1" colspan="4"><center><?php if($page == 'request'){echo '<font color="red"><b>Reliever Request</font></b>';}else{echo '<font color="red"><b>Submitted</font></b>';}?> Leaves<br></center></td>
                                </tr>
                                <tr>
                                    <td colspan="4"><hr></td>
                                </tr>
