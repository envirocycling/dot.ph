...
$sql_user = mysql_query("SELECT * from users WHERE user_id='$userId'");
$row_user = mysql_fetch_array($sql_user);
if (isset($_POST['submit'])) {
    if ($row_user['class'] == 'external') {
        $query = "SELECT * FROM payment WHERE status='approved' and date>='" . $_POST['from'] . "' and date<='" . $_POST['to'] . "' and branch_code NOT LIKE '%" . $_SESSION['branch'] . "%' and printed='1'";
    } else {
        $query = "SELECT * FROM payment WHERE status='approved' and date>='" . $_POST['from'] . "' and date<='" . $_POST['to'] . "' and branch_code like '%" . $_SESSION['branch'] . "%' and printed='1'";
    }
} else {
    if ($row_user['class'] == 'external') {
        $query = "SELECT * FROM payment WHERE status='approved' and printed='1' and branch_code NOT LIKE '%" . $_SESSION['branch'] . "%'";
    } else {
        $query = "SELECT * FROM payment WHERE status='approved' and printed='1' and branch_code LIKE '%" . $_SESSION['branch'] . "%'";
    }
}
