...
<div class="grid_10">
    <div class="box round first">
        ...
        <table class="data display datatable" id="example">
            ...
            $branch=$_SESSION['branch'];
            include("config.php");
            $query="SELECT * FROM requests WHERE type='electric_equipment' and mecha_signature='' and status='' order by status desc";
            ...
            while($row = mysql_fetch_array($result)) {
                ...
                echo'<td class="data"><a  href=electricTOSignPrintPR.php?request_id=' . $row["request_id"] . '><button>' . 'View' . '</button><a href=electricToSignCancel.php?request_id=' . $row["request_id"] . '><button>' . 'Disapprove' . '</button></a></a>';
                echo '</td>';
                echo '</td>';
                echo "</tr>";
            }
