...
if ($username == 'lorna_regala') {
    ...
    echo "window.location='hrPending.php';";
    echo "</script>";
} else if ($username == 'canvasser' || $username == 'ron') {
    header('Location:canvassing_home.php');
    //header('Location:canvasser.php');
    echo "<script>";
    echo "window.location='canvassing_home.php';";
    //echo "window.location='canvasser.php';";
    echo "</script>";
...
<?php if ($_SESSION['position'] == 'BH') { ?>
    ...
    <div class="grid_10">
        <div class="box round first">
            ...
            <table class="data display datatable" id="example">
                ...
                $branch = $_SESSION['branch'];
                include("config.php");
                $query = "SELECT * FROM requests where (status='' or status='for approval') and verified='" . $_SESSION['name'] . "' order by status desc";
    ...
    <?php
    include ('config.php');
    $result = mysql_query("SELECT count(request_id) FROM requests where status ='pending to Allen' And notipending='0'");
    while ($row = mysql_fetch_array($result)) {
        $dbCount = $row['count(request_id)'];
        if ($dbCount > 0 ) {

			?>
           <script> sNotify.addToQueue("You currently have   <?php echo $dbCount ;?>   <a href='prRequests_afr.php'><u><i><b>pending</u></i></b> </a> to Approved PR Requests.");</script>;
        <?php 
		}
