...
if (mysql_query("UPDATE check_req SET date_submitted='" . $_POST['date_submitted'] . "', payee='" . $_POST['payee'] . "', accnt_no='" . $_POST['accnt_no'] . "', amount='" . $_POST['amount'] . "', date_of_check='" . $_POST['date_of_check'] . "', breakdown='" . $_POST['breakdown'] . "', prepared_by='" . $_POST['prepared_by'] . "', audited_by='" . $_POST['audited_by'] . "', approved_by='" . $_POST['approved_by'] . "', submitted_by='" . $_POST['submitted_by'] . "', branch_submitted='" . $_SESSION['branch'] . "' WHERE log_id='" . $_POST['log_id'] . "'")) {
    if ($attachment != 'attachments/') {
        mysql_query("UPDATE check_req SET attachment='$attachment' WHERE log_id='" . $_POST['log_id'] . "'");
    }
    if ($attachment2 != 'attachments/') {
        mysql_query("UPDATE check_req SET attachment2='$attachment2' WHERE log_id='" . $_POST['log_id'] . "'");
    }
    if ($attachment3 != 'attachments/') {
        mysql_query("UPDATE check_req SET attachment3='$attachment3' WHERE log_id='" . $_POST['log_id'] . "'");
    }
    echo "<script>";
