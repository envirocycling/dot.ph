...
<div class="grid_10">
    <div class="box round first">
        ...
        <table class="data display datatable" id="example">
            ...
            include("config.php");
            $query = "SELECT * FROM requests WHERE mecha_signature='' and type='heavy_vehicles' and branch='$branch' and status='for mechanic' order by request_id desc , status desc";
            ...
            while ($row = mysql_fetch_array($result)) {
                ...
                echo'<td class="data"><a  href=mechaTOSignPrintPR.php?request_id=' . $row["request_id"] . '><button>' . 'View' . '</button></a>';
                if ($row['status'] == '') {
                    echo '<a href=mechaToCancel.php?request_id=' . $row["request_id"] . '><button>' . 'Cancel' . '</button></a>';
                }
