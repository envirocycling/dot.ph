...
<html>
    <body>
        <div id="container">
            ...
            echo "<form action='hrToSignForwardPR.php' method='POST'>";
            ...
            $_fullname = $row_verifier['fullname'];
            $_name = $_verified .' - '. $_fullname;

            if ($row['date_verified'] != '') {
                echo "<input type='text' name='verified'  id='verified' value='" .$_name . ' ' . date("M d, Y", strtotime($row['date_verified'])) . "' size=35 readonly>";
            } else {
                echo "<input type='text' name='verified'  id='verified' value='$_name' size=35 readonly>";
            }

            //echo "<input type='text' name='verified' id='verified' value='$_name' readonly>";
...
<div id="buttons">
    <A HREF="javascript:void(0)"
       onclick="window.open('<?php echo $row['quotation'];?>')">
        ...
        echo'<a  href=hrPending.php><button>' . "Back to Home" . '</button></a>';

        //if($_SESSION['position']=='HRD') {

            if($row['status']=='pending') {
                echo '<a href=hrApprovePR.php?request_id=' .$request_id. '><button id="disapprove_button">' . 'Approve' . '</button></a>';
            }else if($row['status']=='' || $row['status']=='for approval') {
                echo '<a href=hrApprovePr2.php?request_id=' .$request_id. '&signature='. $row['verified'] .'><button id="disapprove_button">' . 'Approve' . '</button></a>';
            }
                
            echo'<a href=hrDisapprovedPr.php?request_id=' .$request_id. '><button id="disapprove_button">' . 'Disapprove PR' . '</button></a>';
        //}
