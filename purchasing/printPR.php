...
<html>
    <body>
        <div id="container">
            ...
            $sql_company = mysql_query("SELECT * from branches WHERE branch_name LIKE '%" . $row['branch'] . "%'") or die(mysql_error());
            ...
            if ($row['type'] == 'for approval') {
                ...
                echo "<div id='type'>PR Type: <input type='text'  value='" . ucwords($row['type']) . "' readonly></div>";
            }
            echo '<span id="pr_head">' . strtoupper($row_company['company_name']) . '</span>';
            ...
            echo "<img id='form' src='prform2.png'>";
            $status = $row['status'];
            echo "<input type='text' name='branch' id='branch' value='" . $row['branch'] . "' size='10' readonly>";

            echo "<input type='text' name='date' id='date' size=10 value='" . date("F d, Y", strtotime($row['date'])) . "' readonly>";
            ...
            $sql_verifier = mysql_query("SELECT * from users WHERE name='" . $row['verified'] . "'");
            $row_verifier = mysql_fetch_array($sql_verifier);

            if ($row['date_verified'] != '') {
                echo "<input type='text' name='verified'  id='verified' value='" . $row['verified'] . " - " . $row_verifier['fullname'] . ' ' . date("M d, Y", strtotime($row['date_verified'])) . "' size=35 readonly>";
            } else {
                echo "<input type='text' name='verified'  id='verified' value='" . $row['verified'] . "-". $row_verifier['fullname'] ."' size=35 readonly>";
            }
            ...
            if ($row['date_approved'] != '') {
                echo "<img src='signatures/" . $row['approved'] . ".jpg' id='approved2' width='200' height='80'>";
                echo "<input type='text' name='approved'  id='approved' value='" . $row['approved'] . " - " . $row_approver['fullname'] . ' ' . date("M d, Y", strtotime($row['date_approved'])) . "' size=32 readonly><br>";
            } else {
                echo "<input type='text' name='approved'  id='approved' value='" . $row['approved'] . "-" . $row_approver['fullname'] . "' size=32 readonly><br>";
            }
            ...
            <?php
            if ($status == 'served' || $status == 'approved' && ($row['type'] == 'for approval' || $row['type'] == 'for_sample' || $row['type'] == 'heavy_vehicles' || $row['type'] == 'electric_equipment')) {
//                echo '<img src="signature2.jpg" id="signature" width=200 height=80>';
//                echo "<img src='signatures/" . $row['approved'] . ".jpg' id='approved2' width='200' height='80'>";
            }
            if ($row['verified_signature'] != '') {
                if ($row['type'] == 'heavy_vehicles') {
                    echo "<img src='signatures/RFE.jpg' id='mecha_signature' width=200 height=80>";
                    echo "<img src='signatures/" . $row['verified_signature'] . ".jpg' id='bh_signature2' width=200 height=80>";
                } else if ($row['type'] == 'electric_equipment') {
                    echo "<img src='signatures/RB.jpg' id='mecha_signature' width=200 height=80>";
                    echo "<img src='signatures/" . $row['verified_signature'] . ".jpg' id='bh_signature2' width=200 height=80>";
                } else {
...
<div id="buttons">
    <A HREF="javascript:void(0)" onclick="window.open('<?php echo $row['quotation']; ?>', 'Quotation', 'width=500,height=500')">
        ...
        if ($row['quotation'] != 'quotations/' && $row['quotation'] != '') {
            echo '<button class="noprint" >View Quotation</button></a>';
        } else {
            echo '<button disabled class="noprint" >View Quotation</button></a>';
        }

        if ($row['stamp'] == 'approved' || $row['stamp'] == 'denied') {
            echo "<a href='canvassPR.php?request_id=$request_id'><button>Input Canvass</button></a>";
        }

        if (($_SESSION['user_type'] == 3 && $row['status'] == 'pending to cris')) {
            echo '<a href="pR_ApprovedNew_afr.php?request_id=' . $_GET['request_id'] . '&stat=pending"><button onclick="return confirm(\'Do you want to Approved?\');">Approved</button></a>';
            echo '<a href="pR_ApprovedNew_afr.php?request_id=' . $_GET['request_id'] . '&stat=disapproved"><button  onclick="return confirm(\'Do you want to Disapproved?\');">Dispproved</button></a>';
        }  else if (strtoupper($row['approved']) == strtoupper($_SESSION['username'])) {
            echo '<a href="pR_ApprovedNew_pampIC.php?request_id=' . $_GET['request_id'] . '&stat=approved"><button onclick="return confirm(\'Do you want to Approved?\');">Approved</button></a>';
            echo '<a href="pR_ApprovedNew_pampIC.php?request_id=' . $_GET['request_id'] . '&stat=disapproved"><button  onclick="return confirm(\'Do you want to Disapproved?\');">Dispproved</button></a>';
        }

        echo '<a href="home.php"><button>Back Home</button></a>';
...
<?php
if ($row['stamp'] == 'approved') {
    echo "<img src='img/approved_stamp.png' id='stamp'>";
} else if ($row['stamp'] == 'denied') {
    echo "<img src='img/denied.png' id='stamp'>";
} else if ($row['stamp'] == 'notif') {
    echo "<img src='img/notif.png' id='stamp'>";
}
?>