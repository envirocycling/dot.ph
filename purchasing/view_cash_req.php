...
<center>
    ...
    <h3>
        ...
        if(trim($row['audited_by'])==$_SESSION['name']) {
            echo "<a href='mark_as_audited2.php?id=".$row['log_id']."'><button>Mark as Audited</button></a>";
        }
        ?>
        <?php
        if(trim($row['approved_by'])==$_SESSION['name']) {
            echo "<a href='mark_as_approved2.php?id=".$row['log_id']."'><button>Mark as Approved</button></a>";
        }
        ?>
