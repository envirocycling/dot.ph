...
<div class="grid_10">
    <div class="box round first">
        ...
        <table class="data display datatable" id="example">
            ...
            include("config.php");
            $query="SELECT * FROM requests where status='disapproved' and verified='".$_SESSION['name']."' order by status desc";
            ...
            while($row = mysql_fetch_array($result)) {
                ...
                echo'<td class="data"><a  href=bhTOSignPrintPR.php?request_id=' . $row["request_id"] . '><button>' . 'View' . '</button><a href=cancel.php?request_id=' . $row["request_id"] . '><button>' . 'Cancel' . '</button></a></a>';
                echo '</td>';
                echo '</td>';
                echo "<td>";
                echo "<span id='items'>";
