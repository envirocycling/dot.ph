        ...
        <table class="data display datatable" id="example">
            ...
            if (isset($_POST['submit'])) {
                if ($_POST['status'] == 'processed') {
                    $sql = mysql_query("SELECT * FROM cash_req WHERE audited_by2='" . $_SESSION['name'] . "' and status='" . $_POST['status'] . "' and deposited_by!=''");
                } else {
                    $sql = mysql_query("SELECT * FROM cash_req WHERE audited_by2='" . $_SESSION['name'] . "' and status='" . $_POST['status'] . "' and deposited_by=''");
                }
            } else {
                $sql = mysql_query("SELECT * FROM cash_req WHERE audited_by2='" . $_SESSION['name'] . "' and status='audited' and deposited_by=''");
            }
            ...
            while ($row = mysql_fetch_array($sql)) {
                ...
                if ($row['status'] == 'audited') {
                    echo " <a href='new_audit_cash_req.php?id=" . $row['log_id'] . "&type=auditor2'><button onclick=\"return confirm('Are you sure you approve this request? once you click [OK] you cant undo your action.')\">Approve</button></a>";
