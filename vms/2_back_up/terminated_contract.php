...
<html lang=''>
    ...
    <center>
        <div id="body">
            ...
            $sql_contract = mysql_query("SELECT * from tbl_contract WHERE status LIKE 'approved%'") or die(mysql_error());
            while ($row_contract = mysql_fetch_array($sql_contract)) {
                ...
                if (mysql_num_rows($sql_truck) > 0) {
                    mysql_query("UPDATE tbl_contract SET branch_noti='1' WHERE id='" . $row_contract['id'] . "'") or die(mysql_error());
                }
            }
            ?>
            <?php //startofcode===========================================================================?>
...
while ($row_select = mysql_fetch_array($select)) {
    include('connect.php');
    $truck = mysql_query("SELECT * from tbl_truck_report WHERE id = '" . $row_select['truck_id'] . "' and branch Like '" . $_SESSION['owner'] . "'") or die(mysql_error());
    $row_truck = mysql_fetch_array($truck);
    
   // $history = mysql_query("SELECT * from  tbl_assigntosupp_history WHERE id = '" . $row_select['truck_id'] . "' and branch Like '" . $_SESSION['owner'] . "'") or die(mysql_error());
    //$row_history = mysql_fetch_array($truck);
...
?>
            ...
            <?php
                include('connect.php');
                $sql_contractlist = mysql_query("SELECT * from tbl_assigntosupp_history WHERE remarks LIKE '%Terminated Contract%'") or die(mysql_error()); 
            ?>
                ...
                <tr>
                    <td>
                        ...
                        while($row_contractlist = mysql_fetch_array($sql_contractlist)){
                            include('connect.php');
                            $sql_vehicle = mysql_query("SELECT * from tbl_truck_report WHERE id = '".$row_contractlist['truckid']."' and branch LIKE '%".$_SESSION['owner']."%' ") or die(mysql_error());
                            $row_vehicle = mysql_fetch_array($sql_vehicle);
                            
                            $sql_branch = mysql_query("SELECT * from tbl_branches WHERE branch_name LIKE '%".$_SESSION['owner']."%'") or die(mysql_error());
                            ...
                            echo '<tr>
                                ...
                                <td class="data">'.$row_supplier['branch'].'</td>
