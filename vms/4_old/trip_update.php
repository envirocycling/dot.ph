...
if(mysql_num_rows($up) > 0){
	...
	$update = mysql_query("Update tbl_trip SET supplier='$supplier',ton='$ton',ins='$out',outs='$in',refill='$refill',driver='$driver',helper ='$helper',remarks='$remarks',cost='$cost',updates='1',no='$month',km='$km',kmlit='$kmlit' Where truckid='$pno' And day='$days' And month='$month' And year='$year' ") or die (mysql_error());
...
$insert = mysql_query("Insert into tbl_trip (truckid,day,month,year,supplier,ton,ins,outs,refill,driver,helper,remarks,cost,updates,no,km,kmlit)
Values ('$pno','$days','$month','$year','$supplier','$ton','$out','$in','$refill','$driver','$helper','$remarks','$cost','1','$month1','$km','$kmlit')") or die (mysql_error());
