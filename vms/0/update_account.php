...
<?php 
include('connect.php');
$select = mysql_query("Select * from tbl_users Where username='".$_SESSION['username']."'") or die(mysql_error());
	...
	if($row['password'] != $_POST['oldpassword']){
        ...
        <?php
	}else if($_POST['username'] == $_SESSION['username'] && $_POST['password'] == $row['password']){
		...
		$update = mysql_query("Update tbl_users Set username='".strtoupper($_POST['username'])."', password='".$_POST['password']."' Where username='".$_SESSION['username']."'") or die (mysql_error());
