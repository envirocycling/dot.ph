...
.mytd2:hover{
	background-color:#d3e9ff;
		...
		<tr class="<?php echo $css;?>">
            ...
            <td align="center" width="10%"><?php if(!empty($row['oil'])){ echo $next;}?></td>
            <td align="center"><font size="-2">
			<?php if($_SESSION['owner'] == $row['branch'] || $_SESSION['owner'] == 'PAMPANGA'){?><a href="m_changeoil.php?id=<?php echo $row['id'];?>"><input type="button" value="View/Update"></a><?php }?></td>
			<?php  $repair = date("M d,y",strtotime($repair_row['date']));
			?>
            <td align="center" width="10%"><?php if($repair_row['date']){echo $repair;}?></td>
			<td align="center" width="10%"><font size="-2"><?php if($_SESSION['owner'] == $row['branch'] || $_SESSION['owner'] == 'PAMPANGA'){?><a href="m_repair.php?id=<?php echo $row['id'];?>"><input type="button" value="View/Update"></a><?php }?></td>
		</tr>
...
//$select_date = mysql_query("Select * from tbl_changeoil Where changes <= '$date'") or die (mysql_error());
 //while ($row_truckid = mysql_fetch_array($select_date)){
 		//$select_new = mysql_query("SELECT * from tbl_truck_report WHERE id='".$row_truckid['truckid']."' and branch='".$_SESSION['owner']."'") or die (mysql_error());
	
	//	if(mysql_num_rows($select_new) > 0){
			$count++;
	//	}
// }
