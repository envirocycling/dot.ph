...
$select = mysql_query("Select * from tbl_users Where username='".$_SESSION['encoder_username']."'") or die(mysql_error());
	...
	if($row['password'] != $_POST['oldpassword']){
		...
		$update = mysql_query("Update tbl_users Set username='".$_POST['username']."', password='".$_POST['password']."' Where username='".$_SESSION['encoder_username']."'") or die (mysql_error());
		$_SESSION['encoder_username'] = $_POST['username'];
		?> 
		<script>
