...
<table width="60%" align="center">
	<tr>
		<td colspan="2" align="right"><button onClick="window.open('view_registration_history.php','reg_history','width=400px,height=500px,right=2px,top=100px');">View History</button></td>
	</tr>
<tr><td>
 <table width="60%"  border="1px" align="center" class="CSSTableGenerator">    
		<tr>
			...
			<td align="center" colspan="3" width="17%">Action</td>
...
//echo $year.'-'.$c_year;
	...
	if(!isset($_POST['filter'])){
		$sql_reg = mysql_query("SELECT * from tbl_truckregistration WHERE truckid='".$row['id']."' and status='0'") or die(mysql_error());
	}
  if(mysql_num_rows($sql_reg) > 0 || isset($_POST['filter'])){
  ?>
...
echo '<a href="registration_cancel.php?id='.$row['id'].'">' ?><input type="button" value="Cancel" onClick="return confirm('Do you want to Cancel this Registration?');"></a><?php } ?>
			...
			<td>
				 <input type="button" onClick="window.open('view_orcr.php?id=<?php echo $row['id'];?>','view_orcr','width=900px,height=500px');" value="View OR/CR">
			</td>
		...
		<?php } } 
		if(!isset($_POST['filter'])){
			  $sql_reg= mysql_query("Select * from tbl_truck_report Where class!='HE' and  ending < '$months' order by ending Asc ") or die (mysql_error());
				...
				?>
					<tr>
						...
						<td align="center">
			<?php echo '<a rel="facebox" href="truck_updatereg.php?id='.$row_reg['id'].'">' ?><input type="button" value="Update"/></a> </td><td>
<?php
if($sql_regstatus['status'] == 0) {
	echo '<a  rel="facebox" href="registration_registered.php?id='.$row_reg['id'].'">' ?><input type="button" value="Registered" ></a>
	<?php }else{
echo '<a href="registration_cancel.php?id='.$row_reg['id'].'">' ?><input type="button" value="Cancel" onClick="return confirm('Do you want to Cancel this Registration?');"></a><?php } ?>
			</td>
			<td>
				 <input type="button" onClick="window.open('view_orcr.php?id=<?php echo $row_reg['id'];?>','view_orcr','width=900px,height=500px');" value="View OR/CR">
			</td>
					</tr>
				<?php
				}	
