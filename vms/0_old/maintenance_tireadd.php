...
<?php
include('connect.php');
$select = mysql_query("Select * from tbl_changeswaps Where truckid='".$_GET['p']."' And tireid='1' And remarks='swap' Order by id Desc LIMIT 1") or die (mysql_error());
$row = mysql_fetch_array($select);

$select_swapto = mysql_query("Select * from tbl_changeswaps Where  truckid='".$_GET['p']."' And swapto='1' And remarks='swap' Order by id Desc LIMIT 1") or die (mysql_error());
...
$select_tire= mysql_query("Select * from tbl_trucktires Where truckplate='".@$_GET['p']."' And tireid='1'") or die (mysql_error());}
...
$select_historyc= mysql_query("Select * from tbl_changeswaps Where remarks='change' And tireid='1' order by id Desc LIMIT 1") or die(mysql_error());
$row_historyc = mysql_fetch_array($select_historyc);
$num = mysql_num_rows($select_historyc);


$selects = mysql_query("Select * from tbl_changeswaps Where truckid='".$_GET['p']."' And swapto='1'  And remarks='swap' Order by id Desc LIMIT 1") or die (mysql_error());
...
<input style="width:87%" type="text"  value="1" disabled>
<input  type="hidden" name="tireid" value="1" >
<?php } ?>
</td></tr>
<tr>
<td>Brand:</td>
...
<input type="hidden" name="position" value="<?php echo  $row_tire['position'];?>">
...
<td><br /><?php $selected = mysql_query("Select * from tbl_trucktires Where status='Spare' And truckplate='".$_GET['p']."'") or die(mysql_error());
...
?>
<option value="<?php echo $row_selecteds['id']."-".$row_selecteds['truckplate'];?>"><?php echo $row_selecteds['tirename']."-".$row11['truckplate'];?></option>
<?php } ?>
</select></td>
</td>
<tr id="t2" hidden="">
